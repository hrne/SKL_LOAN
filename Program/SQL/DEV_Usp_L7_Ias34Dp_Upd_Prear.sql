-- 程式功能：清WK檔,FOR維護 Ias34Dp 每月IAS34資料欄位清單D檔
-- 執行時機：每月底日終批次(換日前)
-- 執行方式：由Usp_L7_Ias34Dp_Upd執行
--

-- Work_Ias34DP 資料
create or replace procedure "Usp_L7_Ias34Dp_Upd_Prear" AUTHID CURRENT_USER as
    table_or_view_not_exist exception;
    pragma exception_init(table_or_view_not_exist, -942);
    attempted_ddl_on_in_use_GTT exception;
    pragma exception_init(attempted_ddl_on_in_use_GTT, -14452);
begin
    execute immediate 'drop table "Work_Ias34DP"';
    dbms_output.put_line('drop table Work_Ias34DP');
    execute immediate 'CREATE GLOBAL TEMPORARY TABLE "Work_Ias34DP" (
    ...
    dbms_output.put_line('create table Work_Ias34DP');
    exception 
