-- 程式功能：維護 LoanIfrsHp 每月IFRS9欄位清單8
-- 執行時機：每月底日終批次(換日前)
-- 執行方式：EXEC "Usp_L7_LoanIfrsHp_Upd"(20201231,'System');
--


CREATE OR REPLACE PROCEDURE "Usp_L7_LoanIfrsHp_Upd"
(
    -- 參數
...
BEGIN
  ...
  BEGIN
    ...
    -- 刪除舊資料
    DBMS_OUTPUT.PUT_LINE('DELETE LoanIfrsHp');

    DELETE FROM "LoanIfrsHp"
    WHERE "DataYM" = YYYYMM
      ;

    -- 寫入資料
    DBMS_OUTPUT.PUT_LINE('INSERT LoanIfrsHp');

    INSERT INTO "LoanIfrsHp"
    WITH "Work_HP" AS (
    ...
    SELECT YYYYMM                               AS "DataYM"             -- 年月份
         ...
         , CASE WHEN TRIM(NVL(C."IndustryCode", ' ')) = '' THEN ' '
                ELSE SUBSTR('000000' || TRIM(C."IndustryCode"), -6)
    ...
    DBMS_OUTPUT.PUT_LINE('INSERT LoanIfrsHp END: INS_CNT=' || INS_CNT);
