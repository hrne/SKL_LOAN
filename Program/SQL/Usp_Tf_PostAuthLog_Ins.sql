...
BEGIN
    ...
    SELECT "PO$AARP"."CUSCDT"             AS "AuthCreateDate"      -- 建檔日期 Decimald 8 0
          -- 2023-07-31 Wei from 資料轉換規格驗證 
          -- 盈倩 A02 PO$AARP POASTS 狀態碼 應轉入 (D:暫時取消,R:恢復)
          ,CASE
             WHEN "PO$AARP"."POASTS" = 'D' 
             THEN '9' -- 9:暫停授權
             WHEN "PO$AARP"."POASTS" = 'R' 
             THEN '8' -- 8:恢復授權
           ELSE '1' -- 1:申請
           END                            AS "AuthApplCode"        -- 申請代號，狀態碼 VARCHAR2 1 0
          ...
          ,CASE
             ...
             THEN "PO$AARP"."CUSCDT"
           ELSE 0
           END                            AS "DeleteDate"          -- 暫停授權日期 Decimald 8 0
