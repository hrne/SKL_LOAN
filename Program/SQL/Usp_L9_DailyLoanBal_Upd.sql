...
(
    ...
    TYPE t_dailyloanbal IS  TABLE OF "DailyLoanBal"%rowtype;
    v_dailyloanbal_nt t_dailyloanbal;
    v_dailyloanbal "DailyLoanBal"%rowtype;   
    already_error_log varchar2(1):=0;
    INS_CNT        INT;        -- 新增筆數 
    UPD_CNT        INT;        -- 更新筆數 
    ...
    FROM ( SELECT
     ...
     WHERE M."Seq" = 1
      AND  ( CASE WHEN EOF_YYYYMM > 0  AND  NVL(D."MonthEndYm",0)  > 0  AND  NVL(D."MonthEndYm",0) <>  EOF_YYYYMM
              ...
              END )  > 0      ) loop
    BEGIN   
    v_dailyloanbal."DataDate"     := c."DataDate"; -- 資料日期
    v_dailyloanbal."CustNo"       := c."CustNo"; -- 戶號 
    v_dailyloanbal."FacmNo"       := c."FacmNo"; -- 額度 
    v_dailyloanbal."BormNo"       := c."BormNo"; -- 撥款序號 
    v_dailyloanbal."LatestFlag"   := c."LatestFlag"; -- 資料是否為最新  0.一般變動日 1.最後變動日 
    v_dailyloanbal."MonthEndYm"   := c."MonthEndYm"; -- 月底年月 (yyyymm) 平常日-> 0, 月底日資料 -> yyyymm,ex.202005
    v_dailyloanbal."CurrencyCode" := c."CurrencyCode"; -- 幣別 
    v_dailyloanbal."AcctCode" := c."AcctCode"; -- 業務科目代號 
    v_dailyloanbal."FacAcctCode" := c."FacAcctCode"; -- 額度業務科目
    v_dailyloanbal."ProdNo"      := c."ProdNo"; -- 商品代碼
    v_dailyloanbal."LoanBalance" := c."LoanBalance";--本金餘額 
    v_dailyloanbal."StoreRate"       :=c."StoreRate";  -- 計息利率
    v_dailyloanbal."IntAmtRcv"       := c."IntAmtRcv"; -- 實收利息    轉催收之利息收入AcctCode = '990'
    v_dailyloanbal."IntAmtAcc"       := c."IntAmtAcc"; -- 提存利息    月底日
    v_dailyloanbal."CreateDate"      := c."CreateDate"; -- 建檔日期時間
    v_dailyloanbal."CreateEmpNo"     := c."CreateEmpNo"; -- 建檔人員
    v_dailyloanbal."LastUpdate"      := c."LastUpdate"; -- 最後更新日期時間  
    v_dailyloanbal."LastUpdateEmpNo" := c."LastUpdateEmpNo"; -- 最後更新人員 

    --判斷是否有異動
    INSERT INTO "DailyLoanBal" VALUES v_dailyloanbal;
...
EXCEPTION
WHEN OTHERS THEN
    if (already_error_log =0) then
    ROLLBACK;
      "Usp_L9_UspErrorLog_Ins"(
         'Usp_L9_DailyLoanBal_Upd' -- UspName 預存程序名稱
       , SQLCODE -- Sql Error Code (固定值)
       ,  'update error：' || SQLERRM -- Sql Error Message (固定值)
       , dbms_utility.format_error_backtrace -- Sql Error Trace (固定值)
       , EmpNo -- 發動預存程序的員工編號
       , JobTxSeq -- 啟動批次的交易序號
     );
