...
--   更新 ClCode1	擔保品代號1
    ...
    USING (SELECT DISTINCT
                 ...
                 ,F."ClCode2"
                 ,F."ClNo"
                 ,NVL(CB."CityCode",CL."CityCode") AS "CityCode"
           ...
           WHERE M."YearMonth" = YYYYMM 
             AND NVL(F."CustNo",0) > 0
          ) F
    ON (    M."YearMonth" = YYYYMM
        AND M."CustNo"    = F."CustNo"
        AND M."FacmNo"    = F."FacmNo"
        AND M."BormNo"    = F."BormNo")
    WHEN MATCHED THEN UPDATE SET M."ClCode1" = NVL(F."ClCode1", 0),
                                 M."ClCode2" = NVL(F."ClCode2", 0),
                                 M."ClNo"    = NVL(F."ClNo", 0),
                                 M."CityCode" = F."CityCode";
