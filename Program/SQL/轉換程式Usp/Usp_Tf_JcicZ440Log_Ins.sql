--------------------------------------------------------
--  DDL for Procedure Usp_Tf_JcicZ440Log_Ins
--------------------------------------------------------
set define off;

  CREATE OR REPLACE NONEDITIONABLE PROCEDURE "Usp_Tf_JcicZ440Log_Ins" 
(
    -- 參數
...
BEGIN
    ...
    SELECT "Ukey"              AS "Ukey"                 -- 流水號 VARCHAR2 32 0
          ...
          ,"AgreeDate"         AS "AgreeDate"            -- 同意書取得日期 Decimald 8 0
          ,"StartDate"         AS "StartDate"            -- 首次調解日 Decimald 8 0
          ,"RemindDate"        AS "RemindDate"           -- 債權計算基準日 Decimald 8 0
          ,"ApplyType"         AS "ApplyType"            -- 受理方式 VARCHAR2 1 0
          ,"ReportYn"          AS "ReportYn"             -- 協辦行是否需自行回報債權 VARCHAR2 1 0
    ...
    -- "Usp_Tf_ErrorLog_Ins"(BATCH_LOG_UKEY,'Usp_Tf_JcicZ440Log_Ins',SQLCODE,SQLERRM,dbms_utility.format_error_backtrace);
END;
