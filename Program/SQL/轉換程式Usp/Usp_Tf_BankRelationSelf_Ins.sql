--------------------------------------------------------
--  DDL for Procedure Usp_Tf_BankRelationSelf_Ins
--------------------------------------------------------
set define off;

  CREATE OR REPLACE NONEDITIONABLE PROCEDURE "Usp_Tf_BankRelationSelf_Ins" 
(
    -- 參數
...
BEGIN
    ...
    -- 刪除舊資料
    -- EXECUTE IMMEDIATE 'ALTER TABLE "BankRelationSelf" DISABLE PRIMARY KEY CASCADE';
    EXECUTE IMMEDIATE 'TRUNCATE TABLE "BankRelationSelf" DROP STORAGE';
    -- EXECUTE IMMEDIATE 'ALTER TABLE "BankRelationSelf" ENABLE PRIMARY KEY';
    ...
    FROM "INFOR_BANKRELATIONSELF" S1
    ;
    ...
    -- "Usp_Tf_ErrorLog_Ins"(BATCH_LOG_UKEY,'Usp_Tf_BankRelationSelf_Ins',SQLCODE,SQLERRM,dbms_utility.format_error_backtrace);
END;
