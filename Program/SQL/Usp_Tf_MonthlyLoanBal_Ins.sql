--------------------------------------------------------
--  DDL for Procedure Usp_Tf_MonthlyLoanBal_Ins
--------------------------------------------------------
set define off;
...
BEGIN
    ...
    BEGIN
      ...
      INSERT INTO "MonthlyLoanBal" (
        ...
        , "LoanBalance"         -- 放款餘額 DECIMAL 16 2
        , "MaxLoanBal"          -- 最高放款餘額 DECIMAL 16 2
        , "StoreRate"           -- 計息利率 DECIMAL 6 4
        , "IntAmtRcv"           -- 實收利息 DECIMAL 16 2 用 本月減上月的MSTTIN累計回收利息
        , "IntAmtAcc"           -- 提存利息 DECIMAL 16 2
        , "UnpaidInt"           -- 已到期未繳息 DECIMAL 16 2
        , "UnexpiredInt"        -- 未到期應收息 DECIMAL 16 2
        , "SumRcvInt"           -- 累計回收利息 DECIMAL 16 2
        , "IntAmt"              -- 本月利息 DECIMAL 16 2 本月利息收入= 本月實收利息+本月提存利息-上月提存利息
        ...
        , "AcSubBookCode"       -- 帳冊別 VARCHAR2 3 0
        , "SumRcvPrin"          -- 累計回收本金 DECIMAL 16 2
        , "OvduRcvAmt"          -- 催收還款金額 DECIMAL 16 2
        , "BadDebtAmt"          -- 轉銷呆帳金額 DECIMAL 16 2
        , "PrevPayIntDate"      -- 繳息迄日 DECIMALD 8 
      )
      SELECT S0."ADTYMT"                    AS "YearMonth"           -- 資料年月 DECIMAL 6 0
            ...
            ,S0."MSTTPR"                    AS "SumRcvPrin"          -- 累計回收本金 DECIMAL 16 2
            ,S0."LMSTPN"                    AS "OvduRcvAmt"          -- 催收還款金額 DECIMAL 16 2
            ,S0."LMSFDB"                    AS "BadDebtAmt"          -- 轉銷呆帳金額 DECIMAL 16 2
            ,S0."LMSLPD"                    AS "PrevPayIntDate"      -- 繳息迄日 DECIMALD 8 
      FROM "LA$MSTP" S0
    ...
    -- "Usp_Tf_ErrorLog_Ins"(BATCH_LOG_UKEY,'Usp_Tf_MonthlyLoanBal_Ins',SQLCODE,SQLERRM,dbms_utility.format_error_backtrace);
END;
