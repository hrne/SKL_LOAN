...
-- 執行時機：每月底日終批次(換日前)
-- 執行方式：EXEC "Usp_L8_JcicB093_Upd"(20200420,'999999');
--

CREATE OR REPLACE PROCEDURE "Usp_L8_JcicB093_Upd"
(
    -- 參數
...
BEGIN
  ...
  BEGIN
    ...
    SELECT
         ...
         , NVL(C."OwnerId",' ')                  AS "OwnerId"           -- 擔保品所有權人或代表人IDN/BAN
...
--    Exception
--    WHEN OTHERS THEN
--   "Usp_Tf_ErrorLog_Ins"(BATCH_LOG_UKEY,'Usp_L8_JcicB093_Upd',SQLCODE,SQLERRM,dbms_utility.format_error_backtrace);
  END;
END;
