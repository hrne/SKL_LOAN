...
----
    ...
    SELECT
         ...
         , CASE
             ...
             WHEN NVL("CdCl"."ClTypeJCIC",' ') IN ('25') AND TRIM(NVL("LoanBorMain"."UsageCode", ' ')) IN ('2','3','4') THEN 'M'
             ELSE '1'
...
-- 更新 FeeAmtRcv   本月收取費用 (到額度層)    *************** 只抓 txcd=L3  ref: Project\common\BaTxCom.java  F10+...+F24
    ...
    USING (SELECT "CustNo"
            ...
            WHERE "CustNo"  > 0
             AND  SUBSTR(to_char("AcDate"),1,6)  = to_char(YYYYMM)
             AND  SUBSTR("TitaTxCd",1,2) = 'L3'
             AND  "AcctCode" IN ('F10', 'F29', 'TMI', 'F09', 'F25', 'F07', 'F24')
