...
BEGIN
  ...
  BEGIN
    ...
    SELECT M."CustId"                    AS "CustId"            -- ID
         , NVL(M."IndustryCode", ' ')    AS "IndustryCode"      -- 授信戶行業別
    FROM   "JcicB201" M
    WHERE  M."CustId" IS NOT NULL 
    GROUP BY M."CustId", M."IndustryCode"
      ;
    ...
    SELECT
         ...
         , SUBSTRB(NVL(
             ...
             TRIM(NVL(C."RegNum", ''))  ||
             ( CASE
                 WHEN TRIM(NVL(C."RegNumDash", '') <> '' THEN TRIM(NVL2(C."RegNumDash", '之' || C."RegNumDash", ''))
                 ELSE ''
               END) ||
             '號' ||
...
--               END) ||
         ...
         , NVL(C."IndustryCode",' ')             AS "JobCode"           -- 職業類別  --???
