...
BEGIN
  ...
  BEGIN
    ...
    SELECT G."ApproveNo"                       AS "ApproveNo"     -- 核准號碼
         ...
         , CASE
             WHEN NVL(G."GuaTypeCode",' ') IN ('01','02')      THEN 'G'
             WHEN NVL(G."GuaTypeCode",' ') IN ('03','04')      THEN 'N'
    ...
    SELECT
         ...
         , CASE
             ...
             ELSE 'M'
           END                                    AS "FinCode"           -- 融資分類  --??? 法人:K 其他
                                                                                      --??? 個人:L 購買住宅貸款(非自用) M 購買住宅貸款(自用) N 一般房屋抵押貸款(房屋修繕貸款)
         , 'XX'                                   AS "ProjCode"          -- 政府專業補助貸款分類   --???
         ...
         , 0                                      AS "PrevAmt"           -- 本月應收本金   --???
         , NVL(M."IntAmt",0)                      AS "IntAmt"            -- 本月應收利息
         , 0                                      AS "PrevAmtRcv"        -- 本月實收本金   -- 後面才更新值
         , NVL(M."IntAmtRcv",0)                   AS "IntAmtRcv"         -- 本月實收利息
         ...
         , CASE
             ...
             ELSE  months_between(TO_DATE(TBSDYF,'yyyy-mm-dd'), TO_DATE("LoanBorMain"."NextPayIntDate",'yyyy-mm-dd'))
           END                                    AS "IntDelayMon"       -- 利息逾期月數
         , CASE
             WHEN  NVL("LoanBorMain"."NextRepayDate", 0) = 0
                OR NVL("LoanBorMain"."NextRepayDate", 0) >= TBSDYF THEN 0
             ELSE  months_between(TO_DATE(TBSDYF,'yyyy-mm-dd'), TO_DATE("LoanBorMain"."NextRepayDate",'yyyy-mm-dd'))
           END                                     AS "RepayDelayMon"     -- 本金逾期月數
         , CASE
             WHEN  NVL("LoanBorMain"."MaturityDate", 0) =  0
                OR NVL("LoanBorMain"."MaturityDate", 0) >= TBSDYF THEN 0
             ELSE  months_between(TO_DATE(TBSDYF,'yyyy-mm-dd'), TO_DATE("LoanBorMain"."MaturityDate",'yyyy-mm-dd'))
...
--   更新 PrevAmtRcv  本月實收本金

    DBMS_OUTPUT.PUT_LINE('UPDATE PrevAmtRcv');

    MERGE INTO "JcicMonthlyLoanData" M
    USING (SELECT "CustNo"
                 ,"FacmNo"
                 ,"BormNo"
                 ,SUM("TxAmt")    AS "PrevAmtRcv"
            FROM "AcDetail"             -- 會計帳務明細檔
            WHERE "CustNo"  > 0
             AND  SUBSTR(to_char("AcDate"),1,6)  = to_char(YYYYMM)
             AND  "AcctCode" IN ('310','320','330','340')
             AND  "DbCr" = 'C'
...
--   更新 FeeAmtRcv   本月收取費用
    ...
    DBMS_OUTPUT.PUT_LINE('UPDATE FeeAmtRcv');
    ...
    USING (SELECT "CustNo"
            ...
            WHERE "CustNo"  > 0
             AND  SUBSTR(to_char("AcDate"),1,6)  = to_char(YYYYMM)
             AND  "AcctCode" IN ('F10','F27','F29','F13')
            GROUP BY "CustNo", "FacmNo", "BormNo"
           ) T
    ON (    M."DataYM"   = YYYYMM
        AND M."CustNo"   = T."CustNo"
    ...
    WHEN MATCHED THEN UPDATE SET M."FeeAmtRcv" = T."FeeAmtRcv"
    ;
    UPD_CNT := UPD_CNT + sql%rowcount;
