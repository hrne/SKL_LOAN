...
BEGIN
  ...
  BEGIN
    ...
    -- 寫入資料 Work_B096    -- 從 JcicB090 撈不動產擔保品明細-地號 擔保品編號    (需確認擔保品擷取位置)
    ...
    GROUP BY M."ClActNo", M."LandNo1", M."LandNo2", M."ClTypeJCIC"
      ;
    ...
    SELECT
         ...
         , '92'                                  AS "DataType"                 -- 資料別                        VARCHAR2  2
         , '260'                                 AS "BankItem"                 -- 總行代號                      VARCHAR2  3
         , '2458'                                AS "BranchItem"               -- 分行代號                      VARCHAR2  4
         ...
         , NVL(TRIM("ClLand"."AreaCode"), 0)     AS "AreaJCICCode"             -- 鄉鎮市區別                    DECIMAL   2
         , CASE
         ...
         , CASE
             WHEN TRIM(NVL("ClLand"."LandCode",'')) = '' THEN 'E'  -- 空白
             WHEN "ClLand"."LandCode" IN ('02')          THEN 'A'  -- 田
             WHEN "ClLand"."LandCode" IN ('03')          THEN 'B'  -- 旱
             WHEN "ClLand"."LandCode" IN ('01')          THEN 'C'  -- 建
             WHEN "ClLand"."LandCode" IN ('06')          THEN 'D'  -- 道
             ELSE 'Z'                                              -- 其他
           END                                   AS "LandCode"                 -- 地目                          VARCHAR2  1
         , ROUND(NVL("ClLand"."Area",0)  * 3.3058, 2)
                                                 AS "Area"                     -- 面積                          DECIMAL   10
...
-- 60: 加油站保留地61: 其他保留地
         ...
         , CASE
             WHEN NVL("ClLand"."PostedLandValue",0) < 1000 THEN 1
             ELSE ROUND("ClLand"."PostedLandValue" / 1000, 0)
