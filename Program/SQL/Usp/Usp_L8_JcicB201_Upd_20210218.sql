...
--      LEFT JOIN "LoanBorTx" Tx      ON Tx."CustNo"  = L."CustNo"
--                                   AND Tx."FacmNo"  = L."FacmNo"
--                                   AND Tx."BormNo"  = L."BormNo"
--                                   AND Tx."BorxNo"  = L."LastBorxNo"   -- 最後一筆交易，判斷清償日
...
--            ( M."Status" IN (3,5,9)  AND  TRUNC(Tx."AcDate" / 100) = YYYYMM  )
              ( M."Status" IN (3,5,9)  AND  TRUNC(Tx."AcDate" / 100) = YYYYMM  ) OR
              ( M."Status" IN (3,5,9)  AND  TRUNC(M."PrevPayIntDate" / 100) = YYYYMM  ) OR
              ( M."Status" IN (3,5,9)  AND  TRUNC(M."PrevRepaidDate" / 100) = YYYYMM  )
            )
...
-- 本月還款紀錄
    ...
    WHEN MATCHED THEN UPDATE
      SET M."RepayCode" = CASE
                            ...
                            WHEN greatest(B."IntDelayMon", B."RepayDelayMon") =   0 THEN '0'
                            WHEN greatest(B."IntDelayMon", B."RepayDelayMon") =   1 THEN '1'
                            WHEN greatest(B."IntDelayMon", B."RepayDelayMon") =   2 THEN '2'
                            WHEN greatest(B."IntDelayMon", B."RepayDelayMon") =   3 THEN '3'
                            WHEN greatest(B."IntDelayMon", B."RepayDelayMon") =   4 THEN '4'
                            WHEN greatest(B."IntDelayMon", B."RepayDelayMon") =   5 THEN '5'
                            WHEN greatest(B."IntDelayMon", B."RepayDelayMon") >=  6 THEN '6'
                            ELSE '0'
                          END
      ;
