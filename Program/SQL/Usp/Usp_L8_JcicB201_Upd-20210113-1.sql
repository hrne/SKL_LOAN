  ...
  BEGIN
    ...
    SELECT
         ...
         , CASE
             ...
             ELSE
               CASE
                 WHEN "AcLoanRenew"."CustNo" IS NULL THEN 'A'
                 ELSE 'C'
               END
         ...
         , NVL(M."EvaAmt", 0)                    AS "ClEvaAmt"          -- 擔保品(合計)鑑估值
         , CASE
         ...
         , 0              AS "SmallAmt"          -- 授信餘額列報1（千元）之原始金額（元）
    ...
    FROM   "JcicMonthlyLoanData" M
        LEFT JOIN "AcLoanRenew"  ON "AcLoanRenew"."CustNo"    = M."CustNo"
                                AND "AcLoanRenew"."NewFacmNo" = M."FacmNo"
                                AND "AcLoanRenew"."NewBormNo" = M."BormNo"
