...
-- 執行時機：每月底日終批次(換日前)
-- 執行方式：EXEC "Usp_L8_JcicB090_Upd"(20200331,'CSCHEN');
--
  ...
  BEGIN
    ...
    SELECT
         ...
         , TRIM(to_char(F."ClCode1",'0')) || TRIM(to_char(F."ClCode2",'00')) ||
           TRIM(to_char(F."ClNo",'0000000'))     AS "ClActNo"           -- 擔保品控制編碼    (必須比照 SKL 現行編碼)
...
--    Exception
--    WHEN OTHERS THEN
--   "Usp_Tf_ErrorLog_Ins"(BATCH_LOG_UKEY,'Usp_L8_JcicB090_Upd',SQLCODE,SQLERRM,dbms_utility.format_error_backtrace);
  END;
END;
