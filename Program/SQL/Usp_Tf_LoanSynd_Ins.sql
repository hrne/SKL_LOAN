...
set define off;
  ...
  CREATE OR REPLACE NONEDITIONABLE PROCEDURE "Usp_Tf_LoanSynd_Ins" 
(
    -- 參數
...
BEGIN
    ...
    INSERT INTO "LoanSynd" (
          "SyndNo"              -- 聯貸案序號 DECIMAL 3 0
        , "LeadingBank"         -- 主辦行 VARCHAR2 7 0
        , "AgentBank"           -- 代理行 VARCHAR2 7 0
        , "SigningDate"         -- 簽約日 DECIMALD 8 0
        , "SyndTypeCodeFlag"    -- 國內或國際聯貸 VARCHAR2 1
        , "PartRate"            -- 參貸費率 DECIMAL 16 2
        , "CurrencyCode"        -- 幣別 VARCHAR2 3 0
        , "SyndAmt"             -- 聯貸總金額 DECIMAL 16 2
        , "PartAmt"             -- 參貸金額 DECIMAL 16 2
        , "CreateDate"          -- 建檔日期時間 DATE 0 0
        , "CreateEmpNo"         -- 建檔人員 VARCHAR2 6 0
        , "LastUpdate"          -- 最後更新日期時間 DATE 0 0
        , "LastUpdateEmpNo"     -- 最後更新人員 VARCHAR2 6 0
        , "SyndName"            -- 聯貸案名稱 NVARCHAR2 60
    )
    SELECT ROW_NUMBER() OVER (PARTITION BY S1."LMSACN" ORDER BY S1."LMSACN")
          ...
          ,''                             AS "SyndName"            -- 聯貸案名稱 NVARCHAR2 60
    FROM "TB$ENTP" S1
