 ...
 * @version 1.0.0
 */
public class L2500 extends TradeBuffer {
	private static final Logger logger = LoggerFactory.getLogger(L2500.class);
	...
	public ArrayList<TotaVo> run(TitaVo titaVo) throws LogicException {
		...
		if (iFunCd == 1) {
			...
			// 該戶號部存在客戶主檔 拋錯
			if (tCustMain == null) {
				throw new LogicException(titaVo, "E0005", "L2500 該戶號" + iCustNo + "不存在客戶主檔。");
			}

			tCustRmk = sCustRmkService.findById(custRmkId);
			// 新增時 該戶號,備忘錄序號查有資料 拋錯
			if (tCustRmk != null) {
				throw new LogicException(titaVo, "E0002", "L2500 該戶號,備忘錄序號" + iCustNo + iRmkNo + "已存在於顧客管控警訊檔。");
			}
			...
			tCustRmk = sCustRmkService.holdById(custRmkId);
			if (tCustRmk == null) {
				throw new LogicException(titaVo, "E0004", "L2500 該戶號,備忘錄序號" + iCustNo + iRmkNo + "不存在於顧客管控警訊檔。");
			}

			try {

				this.info(" L2500 deleteCustRmkLog" + tCustRmk);
