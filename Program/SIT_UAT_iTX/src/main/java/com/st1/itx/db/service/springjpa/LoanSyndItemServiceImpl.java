...
public class LoanSyndItemServiceImpl extends ASpringJpaParm implements LoanSyndItemService, InitializingBean {
	...
	public Slice<LoanSyndItem> findAll(int index, int limit, TitaVo... titaVo) {
		...
		if (limit == Integer.MAX_VALUE)
			pageable = PageRequest.of(0, Integer.MAX_VALUE, Sort.by(Sort.Direction.ASC, "SyndNo", "SyndSeq"));
		else
			pageable = PageRequest.of(index, limit, Sort.by(Sort.Direction.ASC, "SyndNo", "SyndSeq"));
	...
	public Slice<LoanSyndItem> findSyndNoEq(int syndNo_0, int index, int limit, TitaVo... titaVo) {
		...
		this.info("findSyndNoEq " + dbName + " : " + "syndNo_0 : " + syndNo_0);
		if (dbName.equals(ContentName.onDay))
			slice = loanSyndItemReposDay.findAllBySyndNoIsOrderBySyndSeqAsc(syndNo_0, pageable);
		else if (dbName.equals(ContentName.onMon))
			slice = loanSyndItemReposMon.findAllBySyndNoIsOrderBySyndSeqAsc(syndNo_0, pageable);
		else if (dbName.equals(ContentName.onHist))
			slice = loanSyndItemReposHist.findAllBySyndNoIsOrderBySyndSeqAsc(syndNo_0, pageable);
		else
			slice = loanSyndItemRepos.findAllBySyndNoIsOrderBySyndSeqAsc(syndNo_0, pageable);
