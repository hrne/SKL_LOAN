...
//		每頁筆數
	...
	private BigDecimal tototheramt = new BigDecimal("0");
	...
	public void exec(TitaVo titaVo) throws LogicException {
		...
		if (fnAllList.size() != 0) {
			...
			for (int j = 1; j <= fnAllList.size(); j++) {
				...
				if ("1".equals(fnAllList.get(i).get("F12")) && !"00103".equals(fnAllList.get(i).get("F11"))) { // 失敗明細
				...
				} else { // 待處理 2 3 4 // 單筆入帳 5 // 批次入帳 6 // 轉暫收 7 // 預先作業 ProcStsCode=1 &&
							// ProcCode=00103
					...
					} else if ("1".equals(ProcStsCode) && "00103".equals(fnAllList.get(i).get("F11"))) {
						othertimes++;
						otheramt = otheramt.add(parse.stringToBigDecimal(fnAllList.get(i).get("F4")));
					}
...
//				扣除總計合計的行數 +1 
	...
	private void amt() {
		...
		this.print(1, 20, "轉暫收　　　　　　　　　　　　　　　件");
		if (otheramt.compareTo(new BigDecimal("0")) != 0) {
			this.print(0, 43, df1.format(otheramt), "R");// 匯款金額
		}
		if (othertimes != 0) {
			this.print(0, 48, String.valueOf(othertimes));// 次數
		}

		amt = amt.add(falseamt).add(falseamt234).add(amt6).add(amt5).add(amt7).add(otheramt);
		times = times + falsetimes + falsetimes234 + times6 + times5 + times7 + othertimes;
	...
	private void totamt() {
		...
		this.print(1, 20, "轉暫收　　　　　　　　　　　　　　　件");
		if (tototheramt.compareTo(new BigDecimal("0")) != 0) {
			this.print(0, 43, df1.format(tototheramt), "R");// 匯款金額
		}
		if (totothertimes != 0) {
			this.print(0, 48, String.valueOf(totothertimes));// 次數
		}

		totamt = totamt.add(totfalseamt).add(totfalseamt234).add(totamt6).add(totamt5).add(totamt7).add(tototheramt);
		totaltimes = totaltimes + totfalsetimes + totfalsetimes234 + tottimes6 + tottimes5 + tottimes7 + totothertimes;
	...
	private void amttototal() {
		...
		tototheramt = tototheramt.add(otheramt);
