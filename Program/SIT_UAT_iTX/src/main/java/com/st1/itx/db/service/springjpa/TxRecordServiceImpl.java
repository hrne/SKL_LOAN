...
public class TxRecordServiceImpl extends ASpringJpaParm implements TxRecordService, InitializingBean {
	...
	@Override
	public Slice<TxRecord> findImportFg(int entdy_0, String brNo_1, String importFg_2, int index, int limit, TitaVo... titaVo) {
		String dbName = "";
		Slice<TxRecord> slice = null;
		if (titaVo.length != 0)
			dbName = titaVo[0].getDataBase() != null ? titaVo[0].getDataBase() : ContentName.onLine;
		...
		if (limit == Integer.MAX_VALUE)
			...
			pageable = PageRequest.of(index, limit);
		this.info("findImportFg " + dbName + " : " + "entdy_0 : " + entdy_0 + " brNo_1 : " + brNo_1 + " importFg_2 : " + importFg_2);
		if (dbName.equals(ContentName.onDay))
			slice = txRecordReposDay.findAllByEntdyIsAndBrNoIsAndImportFgIsOrderByCreateDateAsc(entdy_0, brNo_1, importFg_2, pageable);
		else if (dbName.equals(ContentName.onMon))
			slice = txRecordReposMon.findAllByEntdyIsAndBrNoIsAndImportFgIsOrderByCreateDateAsc(entdy_0, brNo_1, importFg_2, pageable);
		else if (dbName.equals(ContentName.onHist))
			slice = txRecordReposHist.findAllByEntdyIsAndBrNoIsAndImportFgIsOrderByCreateDateAsc(entdy_0, brNo_1, importFg_2, pageable);
		else
			slice = txRecordRepos.findAllByEntdyIsAndBrNoIsAndImportFgIsOrderByCreateDateAsc(entdy_0, brNo_1, importFg_2, pageable);
