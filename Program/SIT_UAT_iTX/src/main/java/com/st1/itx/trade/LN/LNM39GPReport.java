...
public class LNM39GPReport extends MakeReport {
	private static final Logger logger = LoggerFactory.getLogger(LNM39GPReport.class);
	...
	public void exec(TitaVo titaVo) throws LogicException {
		...
		try {
			...
			List<HashMap<String, String>> LNM39GPList = lNM39GPServiceImpl.findAll(titaVo);
...
//			genExcel(titaVo, LNM39GPList);
	...
	private void genFile(TitaVo titaVo, List<HashMap<String, String>> L7List) throws LogicException {
		...
		try {
				...
				for (HashMap<String, String> tL7Vo : L7List) {
					...
					for (int j = 1; j <= tL7Vo.size(); j++) {
						...
						switch (j) {
							...
							case 6 : strField = makeFile.fillStringL(strField,  1, '0');	break;		// 1企業戶/2個人戶
							case 7 : strField = makeFile.fillStringR(strField,  1, ' ');	break;		// 戶況
							case 8 : strField = makeFile.fillStringL(strField,  8, '0');	break;		// 轉催收款日期                   
							case 9 : strField = makeFile.fillStringR(strField,  1, ' ');	break;		// 原始認列時時信用評等     (空白)
							case 10: strField = makeFile.fillStringR(strField,  1, ' ');	break;		// 原始認列時信用評等模型   (空白)
							case 11: strField = makeFile.fillStringR(strField,  1, ' ');	break;		// 財務報導日時信用評等     (空白)
							case 12: strField = makeFile.fillStringR(strField,  1, ' ');	break;		// 財務報導日時信用評等模型 (空白)
							case 13: strField = makeFile.fillStringL(strField,  4, '0');	break;		// 逾期繳款天數                   
							case 14: strField = makeFile.fillStringL(strField,  1, '0');	break;		// IAS39減損客觀條件1 (1=是 2=否)
							case 15: strField = makeFile.fillStringL(strField,  1, '0');	break;		// IAS39減損客觀條件2 (1=是 2=否)
							case 16: strField = makeFile.fillStringL(strField,  1, '0');	break;		// IAS39減損客觀條件3 (1=是 2=否)
							case 17: strField = makeFile.fillStringL(strField,  1, '0');	break;		// IAS39減損客觀條件4 (1=是 2=否)
							case 18: strField = makeFile.fillStringL(strField,  1, '0');	break;		// IAS39減損客觀條件5 (1=是 2=否)
							case 19: strField = makeFile.fillStringR(strField,  1, ' ');	break;		// 內部違約機率降至D評等 (空白)
			...
			// 產製[控制檔]
			makeFile.open(titaVo, titaVo.getEntDyI(), titaVo.getKinbr(), "LNFGPIDX", "LNM39GP 欄位清單７控制檔", "LNFGP.IDX", 1);  // UTF-8
	...
	private void genExcel(TitaVo titaVo, List<HashMap<String, String>> LNList) throws LogicException {
		this.info("=========== LNM39GP genExcel: ");
		this.info("LNM39GP genExcel TitaVo=" + titaVo);
...
//			LNList = lNM39GPServiceImpl.findAll(titaVo);
...
//			this.info("-----------------" + LNList);
			...
			makeExcel.open(titaVo, titaVo.getEntDyI(), titaVo.getKinbr(), "LNFGP", "LNM39GP 欄位清單７", "LNFGP");
			...
			StringWriter errors = new StringWriter();
			e.printStackTrace(new PrintWriter(errors));
			this.info("LNM39GPServiceImpl.genExcel error = " + errors.toString());
		}
	}
