...
//							11.OccSenderRemarker	發動者專用區	X	20	106	戶號+額度
				occursList.putParam("OccSenderRemarker", FormatUtil.padX(
						FormatUtil.pad9("" + result.get("F2"), 7) + FormatUtil.pad9("" + result.get("F6"), 3), 20));
...
//					3	FootNote	備用		X	109	120	
		...
		case 3:
			...
			for (Map<String, String> result : resultList) {
				...
				this.info("AchAuthLogId= " + tAchAuthLogId.toString());
				AchAuthLog tAchAuthLog = achAuthLogService.holdById(tAchAuthLogId);

				// AuthStatus '':新增授權 ' ':再次授權
				if (result.get("F9").isEmpty()) {
					try {
						achAuthLogService.delete(tAchAuthLog);
					} catch (DBException e) {
						throw new LogicException(titaVo, "E0006", "L4040 AchAuthLog " + e.getErrorMsg());
					}
					...
					try {
						...
						throw new LogicException(titaVo, "E0007", "L4040 AchAuthLog " + e.getErrorMsg());
					}
					tTxToDoDetailId = new TxToDoDetailId();
					tTxToDoDetailId.setCustNo(tAchAuthLog.getCustNo());
					tTxToDoDetailId.setFacmNo(tAchAuthLog.getFacmNo());
				...
				Slice<BankAuthAct> slBankAuthAct = bankAuthActService.authCheck(tAchAuthLog.getCustNo(),
						tAchAuthLog.getRepayAcct(), 0, 999, 0, Integer.MAX_VALUE, titaVo);
				if (slBankAuthAct != null) {
					for (BankAuthAct t1 : slBankAuthAct.getContent()) {
						if (t1.getRepayBank().equals(tAchAuthLog.getRepayBank())) {
