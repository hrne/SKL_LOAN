...
public class LoanCalcRepayIntCom extends CommBuffer {
	...
	private void initWorkRoutine() {
		...
		if ((iAmortizedCode == 3 || iAmortizedCode == 4) && (iPrevRepaidDate > 0) && (iNextRepayDate < iIntStartDate) && (iNonePrincipalFlag != 1)) {
	...
	private void adjustAmtRoutine(int wkIndex) throws LogicException {
		...
		if (iAmortizedCode == 3) {
			...
			if (vCalcRepayIntVo.getStoreRate().compareTo(wkBeforeStoreRate) != 0 && "1".equals(iExtraRepayCode)) {
				...
				wkDueAmt = loanDueAmtCom.getDueAmt(vCalcRepayIntVo.getAmount(), vCalcRepayIntVo.getStoreRate(), "3", iFreqBase, wkRestPeriod, wkGracePeriod, iPayIntFreq, iFinalBal, titaVo);
				oDueAmt = wkDueAmt;
