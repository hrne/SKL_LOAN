...
@Repository
public class NegFinShareLogServiceImpl extends ASpringJpaParm implements NegFinShareLogService, InitializingBean {
	@Autowired
	...
	public NegFinShareLog findById(NegFinShareLogId negFinShareLogId, TitaVo... titaVo) {
		...
		this.info("findById " + dbName + " " + negFinShareLogId);
	...
	public Slice<NegFinShareLog> custNoEq(int custNo_0, int index, int limit, TitaVo... titaVo) {
		...
		if (dbName.equals(ContentName.onDay))
			...
			slice = negFinShareLogReposMon.findAllByCustNoIsOrderByCaseSeqDescSeqDescFinCodeAsc(custNo_0, pageable);
		else if (dbName.equals(ContentName.onHist))
			slice = negFinShareLogReposHist.findAllByCustNoIsOrderByCaseSeqDescSeqDescFinCodeAsc(custNo_0, pageable);
		else
			slice = negFinShareLogRepos.findAllByCustNoIsOrderByCaseSeqDescSeqDescFinCodeAsc(custNo_0, pageable);
	...
	public NegFinShareLog holdById(NegFinShareLogId negFinShareLogId, TitaVo... titaVo) {
		...
		if (titaVo.length != 0)
			dbName = titaVo[0].getDataBase() != null ? titaVo[0].getDataBase() : ContentName.onLine;
		this.info("Hold " + dbName + " " + negFinShareLogId);
		Optional<NegFinShareLog> negFinShareLog = null;
		if (dbName.equals(ContentName.onDay))
	...
	public NegFinShareLog holdById(NegFinShareLog negFinShareLog, TitaVo... titaVo) {
		...
		if (titaVo.length != 0)
			dbName = titaVo[0].getDataBase() != null ? titaVo[0].getDataBase() : ContentName.onLine;
		this.info("Hold " + dbName + " " + negFinShareLog.getNegFinShareLogId());
	...
	public void delete(NegFinShareLog negFinShareLog, TitaVo... titaVo) throws DBException {
		...
		if (titaVo.length != 0)
			dbName = titaVo[0].getDataBase() != null ? titaVo[0].getDataBase() : ContentName.onLine;
		this.info("Delete..." + dbName + " " + negFinShareLog.getNegFinShareLogId());
		if (dbName.equals(ContentName.onDay)) {
