 ...
 * @version 1.0.0
 */
public class L4510p extends TradeBuffer {
...
//			F5 ProcCode (流程別)
			...
			tmpFacm tmp2 = new tmpFacm(parse.stringToInteger(result.get("CustNo")), parse.stringToInteger(result.get("FacmNo")), 0, 0, parse.stringToInteger(result.get("Flag")),
...
//			20210409修改為抓取全部撥款且費用，by淑薇電話確認
			if (flagMap2.containsKey(tmp2)) {
				this.info("custNo = " + parse.stringToInteger(result.get("CustNo")) + " facmNo = " + parse.stringToInteger(result.get("FacmNo")) + " 同戶號額度僅進入計算一次 continue...");
				continue;
			} else {
				flagMap2.put(tmp2, 1);
			}
			...
			if ("2".equals(result.get("Flag"))) {
				...
				listBaTxVo = baTxCom.settingUnPaid(iN15EntryDate, parse.stringToInteger(result.get("CustNo")), parse.stringToInteger(result.get("FacmNo")), 0, 1, BigDecimal.ZERO, titaVo);
			} else {
				iY15EntryDate = getList(slEmpDeductSchedule, result.get("AgType1"));
				listBaTxVo = baTxCom.settingUnPaid(iY15EntryDate, parse.stringToInteger(result.get("CustNo")), parse.stringToInteger(result.get("FacmNo")), 0, 1, BigDecimal.ZERO, titaVo);
			}
