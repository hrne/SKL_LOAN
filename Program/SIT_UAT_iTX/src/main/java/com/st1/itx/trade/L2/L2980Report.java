...
public class L2980Report extends MakeReport {
	private static final Logger logger = LoggerFactory.getLogger(L2980Report.class);
	...
	public void exec(TitaVo titaVo, TxBuffer txbuffer) throws LogicException {
		...
		this.print(1, 10, "配偶姓名 . . . .  " + titaVo.getParam("SpouseName"));
	...
	public String DateFormat(String date) {
		String temp = "";
		if(date.length() == 7) {
			temp = date.substring(0,3) + "/" + date.substring(3,5) 
					+ "/" + date.substring(5,7);
		} else {
			temp = date.substring(0,2) + "/" + date.substring(2,4) 
					+ "/" + date.substring(4,6);
		}
