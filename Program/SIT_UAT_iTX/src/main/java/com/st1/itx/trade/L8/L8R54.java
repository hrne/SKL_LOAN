				...
				for (int i = 0; i < testConnect.length; i++) {
					...
					if (connect != null && connect.length() != 0) {
						...
						if (PhoneNo != null && PhoneNo.length() != 0) {
							PhoneNoRemark = "" + connectRemark + " JcicZ048 協商申請日:" + jcicCom.DcToRoc(String.valueOf(JcicZ048Vo.getJcicZ048Id().getRcDate()), 0) + " 報送單位代號:"
									+ JcicZ048Vo.getJcicZ048Id().getSubmitKey() + "";
							break;
						}
					}
				}
			...
			for (JcicZ444 JcicZ444Vo : lJcicZ444) {
				...
				for (int i = 0; i < testConnect.length; i++) {
					...
					if (connect != null && connect.length() != 0) {
						PhoneNo = connect;
						if (PhoneNo != null && PhoneNo.length() != 0) {
							PhoneNoRemark = "" + connectRemark + " JcicZ444 統一款項收付申請日:" + jcicCom.DcToRoc(String.valueOf(JcicZ444Vo.getJcicZ444Id().getApplyDate()), 0) + " 報送單位代號:"
									+ JcicZ444Vo.getJcicZ444Id().getSubmitKey() + "";
							break;
						}
					}
				}
