...
public class LM075Report extends MakeReport {
	private static final Logger logger = LoggerFactory.getLogger(LM075Report.class);
	...
	public boolean exec(TitaVo titaVo) throws LogicException {
		...
		try {
			lLM075 = lM075ServiceImpl.findAll(titaVo, Integer.toString(iAcDate).substring(0,6));
		} catch (Exception e) {
	...
	private void exportExcel(TitaVo titaVo, List<Map<String, String>> lList, int date) throws LogicException {
		...
		makeExcel.open(titaVo, titaVo.getEntDyI(), titaVo.getKinbr(), "LM075", "B041金融機構承作「自然人購置住宅貸款」統計表",
				"LM075_B041金融機構承作「自然人購置住宅貸款」統計表" + showRocDate(entdy, 0).substring(0, 7), "LM075_底稿_B041金融機構承作「自然人購置住宅貸款」統計表.xlsx", 1, "FOA");
			...
			for (Map<String, String> tLDVo : lList) {
				...
				for (int i = 0; i < tLDVo.size(); i++) {
					...
					case 1:
						...
						//           02/04/05 - col 7~10 (+4)
						if (tmpValue.equals("01") || tmpValue.equals("03")) { colShift = 0; } 
						else if (tmpValue.equals("02") || tmpValue.equals("04") || tmpValue.equals("05")) { colShift = 4; } 
						else 
						{
							this.info("LM075Report exportExcel: RuleCodeException, got " + tmpValue);
							break; 
						};
