...
public class LB092Report extends MakeReport {
	...
	public void exec(TitaVo titaVo) throws LogicException {
		...
		try {
			...
			List<Map<String, String>> LBList = lB092ServiceImpl.findAll(titaVo);
//			this.info("-----------------" + LBList);
			if (LBList == null) {
				listCount = 0;
			} else {
	...
	private void genFile(TitaVo titaVo, List<Map<String, String>> LBList) throws LogicException {
		...
		try {
			...
			// 首筆
			strContent = "JCIC-DAT-B092-V01-458" + StringUtils.repeat(" ", 5) + strToday + "01" + StringUtils.repeat(" ", 10) + makeFile.fillStringR("02-23895858#7067", 16, ' ')
					+ makeFile.fillStringR("審查單位聯絡人－許高政", 189, ' ');
