			...
			if (i >= 1 && i <= 18) {
				...
				if (i == 1) {
					...
					if (i == 6) {
						...
						// try {
						// Data=NegCom.NegServiceList1(IntAcDate, IsMainFin, State, Detail,
						// ExportDateYN, IsBtn,titaVo);
						// } catch (LogicException e) {
						// //E5004 讀取DB時發生問題
						// this.info("L5051 ErrorForDB="+e);
						// throw new LogicException(titaVo, "E5004","");
...
//						BufIssue=BufValue(IsMainFin,State,Detail,ExportDateYN,1);
						...
						this.info("\r\nLabelD=[" + LabelD + "],LabelE=[" + LabelE + "],\r\nTodayWorkCnt=[" + TodayWorkCnt + "],TodayWorkAmt=[" + TodayWorkAmt + "],\r\nSummaryWorkCnt=["
								+ SummaryWorkCnt + "],SummaryWorkAmt=[" + SummaryWorkAmt + "]");
						// ThisIssue=Issue[1];//處理事項
...
//		this.totaVo.addOccursList(occursList);
		/*
		 * OORank=9,2 OOLabelA=X,8 OOLabelB=X,8 OOLabelC=X,6 OOLabelD=X,10
		 * OOTodayWorkCnt=9,3 OOTodayWorkAmt=9,14.2 OOLabelE=X,10 OOSummaryWorkCnt=9,3
		 * OOSummaryWorkAmt=9,14.2 OOIssue=X,8 OOTodayIssue=X,6 OOBufToday=X,5
		 * OOBufSummary=X,48 OOBufIssue=X,5 OOBtnCode=9,1
		 */
