 ...
 * @version 1.0.0
 */
public class L420BBatch extends TradeBuffer {
	/* 轉型共用工具 */
	@Autowired
	public Parse parse;
	/* 日期工具 */
...
// 處理代碼 0:入帳 1:刪除 2:訂正 4.刪除回復
		...
		this.info("L420BBatch updateHead batxExeCode = " + batxExeCode + ", unCheckCnt=" + unCheckCnt
				+ ", checkErrorCnt=" + checkErrorCnt + ", toDoCnt=" + toDoCnt + ", doneCnt=" + doneCnt);
		...
		try {
			...
			throw new LogicException(titaVo, "E0007", "BatxHead " + tBatxHead + e.getErrorMsg());
		}
...
// ProcStsCode 處理狀態 0.未檢核 1.不處理 2.人工處理 3.檢核錯誤 4.檢核正常 5.人工入帳 6.批次入帳 7.虛擬轉暫收
	...
	private void excuteTx(int Hcode, BatxDetail tDetail, TitaVo titaVo) throws LogicException {
		...
		// 執行入帳交易
		this.info("L420BBatch excuteTx " + txTitaVo);
