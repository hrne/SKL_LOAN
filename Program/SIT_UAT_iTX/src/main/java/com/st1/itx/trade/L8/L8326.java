...
public class L8326 extends TradeBuffer {
	...
	public ArrayList<TotaVo> run(TitaVo titaVo) throws LogicException {
		...
		// 1.2 「IDN+報送單位代號+調解申請日+受理調解機構代號」若未曾報送過「'440':前置調解受理申請暨請求回報債權通知資料」，予以剔退處理.(交易代碼為'X'者不檢核)
		...
		// 1.4 同一KEY值報送「'447':金融機構無擔保債務協議資料」後，若再報送本檔案資料時，結案理由代碼僅能報送'00','01','90'及'99'，其餘結案理由皆以剔退處理.
		...
		if("00".equals(iCloseCode)) {
			...
			if (iJcicZ451 != null) {
				...
				if (txDate <= formateDelayYM) {
					throw new LogicException("E0005", "於'451':延期繳款期間(" + iJcicZ451.getDelayYM() + "前)不可報送「結案原因代號」 為'00'之本檔案資料.");
				}
			} 
		}// 1.5 end
		
		// 2 各資料檔案格式若第2欄「交易代碼」無「'D'刪除」功能者，如有資料key值報送錯誤情形者，需以本檔案格式報送「結案原因代號'97':資料key值報送錯誤，本行結案」至本中心，並重新報送更正後資料.***
