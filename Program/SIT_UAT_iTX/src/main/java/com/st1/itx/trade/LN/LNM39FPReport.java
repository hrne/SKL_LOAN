...
public class LNM39FPReport extends MakeReport {
	private static final Logger logger = LoggerFactory.getLogger(LNM39FPReport.class);
	...
	public void exec(TitaVo titaVo) throws LogicException {
		...
		try {
			...
			List<HashMap<String, String>> LNM39FPList = lNM39FPServiceImpl.findAll(titaVo);
...
//			genExcel(titaVo, LNM39FPList);
	...
	private void genFile(TitaVo titaVo, List<HashMap<String, String>> L7List) throws LogicException {
		...
		try {
				...
				for (HashMap<String, String> tL7Vo : L7List) {
					...
					for (int j = 1; j <= tL7Vo.size(); j++) {
						...
						switch (j) {
							...
							case 3 : strField = makeFile.fillStringL(strField, 3, '0');		break;		// 協議編號
							case 4 : strField = makeFile.fillStringR(strField, 1, ' ');		break;		// 協議前後
							case 5 : strField = makeFile.fillStringL(strField, 3, '0');		break;		// 額度編號
							case 6 : strField = makeFile.fillStringL(strField, 3, '0');		break;		// 撥款序號
			...
			// 產製[控制檔]
			makeFile.open(titaVo, titaVo.getEntDyI(), titaVo.getKinbr(), "LNFFPIDX", "LNM39FP 欄位清單６控制檔", "LNFFP.IDX", 1);  // UTF-8
	...
	private void genExcel(TitaVo titaVo, List<HashMap<String, String>> LNList) throws LogicException {
		this.info("=========== LNM39FP genExcel: ");
		this.info("LNM39FP genExcel TitaVo=" + titaVo);
...
//			LNList = lNM39FPServiceImpl.findAll(titaVo);
...
//			this.info("-----------------" + LNList);
			...
			makeExcel.open(titaVo, titaVo.getEntDyI(), titaVo.getKinbr(), "LNFFP", "LNM39FP 欄位清單６", "LNFFP");
			...
			StringWriter errors = new StringWriter();
			e.printStackTrace(new PrintWriter(errors));
			this.info("LNM39FPServiceImpl.genExcel error = " + errors.toString());
		}
	}
