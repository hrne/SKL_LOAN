...
public class LNM39FPReport extends MakeReport {
	...
	public boolean exec(TitaVo titaVo) throws LogicException {
		...
		List<Map<String, String>> LNM39FPList = null;
		try {
			LNM39FPList = lNM39FPServiceImpl.findAll(dataMonth, titaVo);
		} catch (Exception e) {
			StringWriter errors = new StringWriter();
			e.printStackTrace(new PrintWriter(errors));
			this.error("LNM39FPReport LNM39FPServiceImpl.findAll error = " + errors.toString());
			return false;
		}
		...
		try {
			...
			StringWriter errors = new StringWriter();
			e.printStackTrace(new PrintWriter(errors));
			this.error("LNM39FPReport.genFile error = " + errors.toString());
			return false;
		}
	...
	private void genFile(TitaVo titaVo, List<Map<String, String>> L7List) throws LogicException {
		...
		try {
			...
			// 產製[控制檔]
			makeFileC.open(titaVo, titaVo.getEntDyI(), titaVo.getKinbr(), "LNFFPIDX", "LNM39FP 欄位清單６控制檔", "LNFFP.IDX",
					1); // UTF-8
