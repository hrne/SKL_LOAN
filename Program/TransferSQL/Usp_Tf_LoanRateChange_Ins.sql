...
set define off;
  ...
  CREATE OR REPLACE PROCEDURE "Usp_Tf_LoanRateChange_Ins" 
(
    -- 參數
...
BEGIN
    ...
    */
    ...
    SELECT LA."LMSACN"                    AS "CustNo"              -- 借款人戶號 DECIMAL 7 0
          ...
          ,0                              AS "IndividualIncr"      -- 個別加碼利率 DECIMAL 6 4
          ,CB."BaseRate" + LA."ASCRAT"    AS "FitRate"             -- 適用利率 DECIMAL 6 4
          ,''                             AS "Remark"              -- 備註 NVARCHAR2 60 0
          ,0                              AS "AcDate"              -- 交易序號-會計日期 DECIMALD 8 0
          ,''                             AS "TellerNo"            -- 交易序號-櫃員別 VARCHAR2 6 0
          ,''                             AS "TxtNo"               -- 交易序號-流水號 VARCHAR2 8 0
    ...
    WHERE LA."ASCADT" > "TbsDyF" -- 未來的
      AND NVL(LM."LMSACN",0) > 0 -- 有串到放款主檔的資料才寫入
      AND NVL(LRC."EffectDate",0) = 0 -- 尚未被記錄在放款利率變動檔的加碼利率才寫入
      AND FP."BaseRateCode" IN ('01','02')
      AND CB."Seq" = 1
    ;
