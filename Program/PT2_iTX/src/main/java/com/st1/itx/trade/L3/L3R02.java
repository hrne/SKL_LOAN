...
public class L3R02 extends TradeBuffer {
	...
	public ArrayList<TotaVo> run(TitaVo titaVo) throws LogicException {
		...
		if (tLoanBorMain == null) {
			if (iBormNo > 900) {
				throw new LogicException(titaVo, "E0001", " 放款主檔 戶號 = " + iCustNo + " 額度編號 =  " + iFacmNo + " 預約序號 = " + iBormNo); // 查詢資料不存在
			} else {
		...
		if (tLoanBorMain.getActFg() == 1 && iFKey == 0) {
			throw new LogicException(titaVo, "E0021", " 放款主檔 戶號 = " + iCustNo + " 額度編號 =  " + iFacmNo + " 撥款序號 = " + iBormNo); // 該筆資料待放行中
		}
		if (iBormNo <= 900) {
