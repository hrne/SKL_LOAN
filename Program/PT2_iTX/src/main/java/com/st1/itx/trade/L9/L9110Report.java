		...
		if (tL9110 != null && tL9110.size() != 0) {
			...
			if (tL9110.get("F58").isEmpty() || "999".equals(tL9110.get("F58")) || "000".equals(tL9110.get("F58"))) {
				...
				if ("001".equals(tL9110.get("F58")) || "002".equals(tL9110.get("F58"))) {
					...
					this.print(0, 35, "綁約期限 ..... " + tL9110.get("F48") + " 個月");
					this.print(0, 69, "違約分段月數 . " + tL9110.get("F62") + " 個月");
				}
			}
...
//					}
	...
	private void fillDataNaturalPerson(TitaVo titaVo) throws LogicException {
		...
		if (tL9110 != null && tL9110.size() != 0) {
			...
			this.print(0, 138, "員工代號 ..... " + tL9110.get("F9"));
			CdEmp tCdEmp = sCdEmpService.findById(tL9110.get("F9"), titaVo);
			if (tCdEmp != null) {
				if (employeeCom.isDay15Employee(tCdEmp, titaVo)) {
					this.print(1, 5, "15 日薪 ...... " + "十五日薪");
				} else {
					this.print(1, 5, "15 日薪 ...... " + "非十五日薪");
				}
			} else {
				this.print(1, 5, "15 日薪 ...... " + tL9110.get("F10"));
			}
			this.print(0, 50, "配偶姓名 ..... " + isNullorEmpty(tL9110.get("F5"), titaVo));
			this.print(0, 99, "配偶統一編號 . " + isNullorEmpty(tL9110.get("F6"), titaVo));
...
//		print(0, 141, formatAmt(computeDivide(totalSettingAmt, thousand, 0), 0), "R");
	...
	private void printBuildingLandDetail(List<Map<String, String>> listBuildingQuery, String lineAmt)
		 ...
		 */
		checkSpace(5);
		print(1, 2, "　　　　　　土地　　　　　　　　提供人／土地坐落　　　　　　　　　　　　　　　　　地號　　　　公設　　　　（坪）　　（坪）　　（坪）　（坪）　（坪）　（坪）　（仟）");
		print(1, 2, "擔保品別　　序號　擔保品編號　　提供人／門牌號碼　　　　　　　　　　　　　　　　　主建物建號　建號　主建物面積　土地面積　附屬建物　　公設　　車位　　核貸　　設定　");
		print(1, 2, "－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－");
		...
		int lastlandCnt = 0;
		for (Map<String, String> mBuilding : listBuildingQuery) {
			if ("1".equals(mBuilding.get("F1")) && "1".equals(mBuilding.get("F2"))) {
				bdCnt++;
			}
			if ("0".equals(mBuilding.get("F4"))) {
				landCnt++;
			}
		}
		...
		for (Map<String, String> mBuilding : listBuildingQuery) {
			...
			if ("2".equals(mBuilding.get("F1"))) {
				totalLandArea = parse.stringToBigDecimal(mBuilding.get("F29"));
			}
			...
			if ("1".equals(mBuilding.get("F0")) || "1".equals(mBuilding.get("F1"))) {
				clItem = mBuilding.get("F3"); // 擔保品別
			}
			...
			if ("2".equals(mBuilding.get("F1"))) {
				...
				this.print(0, 32, mBuilding.get("F6")); // 提供人
			}
			this.print(0, 84, mBuilding.get("F8")); // 主建物建號
			this.print(0, 94, mBuilding.get("F9")); // 公設建號
			if ("1".equals(mBuilding.get("F1"))) {
				this.print(0, 112, mBuilding.get("F10").trim().isEmpty() ? " " : formatAmt(mBuilding.get("F10"), 2),
						"R"); // 主建物面積
				if (!mBuilding.get("F10").trim().isEmpty()) {
					totalFloorArea = totalFloorArea.add(getBigDecimal(mBuilding.get("F10")));
				}
			}

			if ("2".equals(mBuilding.get("F1"))) {
				this.print(0, 123, mBuilding.get("F10").trim().isEmpty() ? " " : formatAmt(mBuilding.get("F10"), 2),
						"R"); // 土地面積
			}
			this.print(0, 132, mBuilding.get("F11").trim().isEmpty() ? " " : formatAmt(mBuilding.get("F11"), 2), "R"); // 附屬建物面積
			if (!mBuilding.get("F11").trim().isEmpty()) {
				totalBdSubArea = totalBdSubArea.add(getBigDecimal(mBuilding.get("F11")));
			}
			this.print(0, 140, mBuilding.get("F12").trim().isEmpty() ? " " : formatAmt(mBuilding.get("F12"), 2), "R"); // 公設面積
			if (!mBuilding.get("F12").trim().isEmpty()) {
				totalPublicArea = totalPublicArea.add(getBigDecimal(mBuilding.get("F12")));
			}
			this.print(0, 148, mBuilding.get("F13").trim().isEmpty() ? " " : formatAmt(mBuilding.get("F13"), 2), "R"); // 車位面積
			if (!mBuilding.get("F13").trim().isEmpty()) {
				totalCarArea = totalCarArea.add(getBigDecimal(mBuilding.get("F13")));
			}
			if (("1".equals(mBuilding.get("F1")) && "1".equals(mBuilding.get("F2")))
					|| "0".equals(mBuilding.get("F4"))) {
				this.print(0, 156,
						"1".equals(mBuilding.get("F1")) ? formatAmt(bdloanAmt, 0) : formatAmt(landloanAmt, 0), "R"); // 核貸
			}
			if (("1".equals(mBuilding.get("F1")) && "1".equals(mBuilding.get("F2")))
					|| ("2".equals(mBuilding.get("F1")) && "0".equals(mBuilding.get("F4")))) {
				this.print(0, 164, mBuilding.get("F14").trim().isEmpty() ? " "
						: formatAmt(computeDivide(getBigDecimal(mBuilding.get("F14")), thousand, 0), 0), "R");// 設定金額
			}

			if ((("1".equals(mBuilding.get("F1")) && "1".equals(mBuilding.get("F2")))
					|| ("2".equals(mBuilding.get("F1")) && "0".equals(mBuilding.get("F4")))
							&& !mBuilding.get("F10").trim().isEmpty())) {
				totalSettingAmt = totalSettingAmt.add(getBigDecimal(mBuilding.get("F14")));
			}
			if ("1".equals(mBuilding.get("F1"))) {
				this.print(1, 1, " ");
				this.print(0, 32, mBuilding.get("F7")); // 門牌
			}
		}
		...
		print(1, 2, "－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－");
		print(1, 5, " 　　合計：");
		print(0, 112, formatAmt(totalFloorArea, 2), "R");
		print(0, 123, formatAmt(totalLandArea, 2), "R");// 土地總面積
		print(0, 132, formatAmt(totalBdSubArea, 2), "R");
		print(0, 140, formatAmt(totalPublicArea, 2), "R");
		print(0, 148, formatAmt(totalCarArea, 2), "R");
		print(0, 156, formatAmt(loanAmttotal, 0), "R");
		print(0, 164, formatAmt(computeDivide(totalSettingAmt, thousand, 0), 0), "R");
	...
	private void printCrossUse(List<Map<String, String>> listCrossUseQuery) {
		if (listCrossUseQuery == null || listCrossUseQuery.isEmpty()) {
			...
			int i = 0;
			int j = 0;
			String displaySubCompany = "";
			String displaySubCompanyX = "";
			for (Map<String, String> queryCrossUse : listCrossUseQuery) {
				if (!displaySubCompany.isEmpty()) {
					displaySubCompany += "、";
				}
				displaySubCompanyX = displaySubCompany + queryCrossUse.get("F0");
				if (displaySubCompanyX.length() > 65) {
					this.print(i, 20, displaySubCompany);
					displaySubCompany = "";
					i = 1;
					j = 0;
				}
				displaySubCompany += queryCrossUse.get("F0");
				j++;
			}
			this.print(i, 20, displaySubCompany);
		}
...
//		this.print(1, 3, "　－－－－　－－　－－－－－－－－　－－　－－－－－－　－－－－－");
		...
		if (listCustTelNoQuery == null || listCustTelNoQuery.isEmpty()) {
			...
			for (Map<String, String> queryCustTel : listCustTelNoQuery) {
				String telType = queryCustTel.get("F0").trim();
				String telArea = queryCustTel.get("F1").trim();
				// 區碼加-
				if (!telArea.isEmpty()) {
					telArea += "-";
				}
				String telNo = queryCustTel.get("F2").trim();
				String telExt = queryCustTel.get("F3").trim();
				telTextX = telText + telType + " ... " + telArea + " " + telNo + " " + telExt;

				if (telTextX.length() > 130) {
					this.print(i, 5, telText);
					telText = "";
					j = 0;
					i = 1;
				}
				telText += telType + " ... " + telArea + " " + telNo + " " + telExt + "　"; // 電話種類
//				this.print(0, 45, queryCustTel.get("F4")); // 與借款人關係 	qc2446不需要
//				this.print(0, 59, queryCustTel.get("F5")); // 聯絡人姓名	qc2446不需要
				j++;
