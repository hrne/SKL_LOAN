...
public class LP006Report extends MakeReport {
	...
	public void exec(TitaVo titaVo) throws LogicException {
		...
		// 年度
		workSeasonYY = parse.stringToInteger(titaVo.get("WorkSeasonYY"));
		workSeasonSS = parse.stringToInteger(titaVo.get("WorkSeasonSS"));
		...
		if ("1".equals(titaVo.getParam("FunCd"))) {
			exportChangeExcel(effectiveDateS, effectiveDateE, titaVo); // 異動清冊
		} else {
			exportAllExcel(effectiveDateS, effectiveDateE, titaVo); // 職級名冊

		}
		webClient.sendPost(dDateUtil.getNowStringBc(), "2300", titaVo.getTlrNo(), "Y", "LC009",
				titaVo.getTlrNo() + "LP006", "", titaVo);
