...
//	每頁筆數
	...
	public long exec(TitaVo titaVo1, List<Map<String, String>> fnAllList) throws LogicException {
...
//		}
		...
		if (fnAllList.size() > 0) {
			...
			ReportVo reportVo = ReportVo.builder().setBrno(titaVo.getBrno()).setRptDate(titaVo.getEntDyI()).setRptCode("L4520")
					.setRptItem("火險費沖銷明細表（員工扣薪）").setRptSize("A4").setSecurity("").setPageOrientation("L").build();
