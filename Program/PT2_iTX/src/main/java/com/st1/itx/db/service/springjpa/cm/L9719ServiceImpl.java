...
public class L9719ServiceImpl extends ASpringJpaParm implements InitializingBean {
	...
	public List<Map<String, String>> findAll(TitaVo titaVo) throws Exception {
		...
		sql += "            ,SUM(ROUND(CASE WHEN DECODE(NVL(MLB.\"AcctCode\", ' '), '990', '990', 'OTHER') = '990' ";
		sql += "                       AND I.\"YearMonth\" = :inputYearMonth ";
		sql += "                 THEN NVL(I.\"AccumDPAmortized\", 0)";
		sql += "                 ELSE 0 END";
		sql += "                )";
		sql += "             -";
		sql += "             ROUND(CASE WHEN DECODE(NVL(MLB.\"AcctCode\", ' '), '990', '990', 'OTHER') = '990' ";
		sql += "                       AND I.\"YearMonth\" = :lastYearMonth";
		sql += "                 THEN NVL(I.\"AccumDPAmortized\", 0)";
		sql += "                 ELSE 0 END";
		sql += "                ))";
		sql += "             AS \"OvAmt\"";
		sql += "            ,SUM(ROUND(CASE WHEN DECODE(NVL(MLB.\"AcctCode\", ' '), '990', '990', 'OTHER') = 'OTHER' ";
		sql += "                       AND I.\"YearMonth\" = :inputYearMonth ";
		sql += "                 THEN NVL(I.\"AccumDPAmortized\", 0)";
		sql += "                 ELSE 0 END";
		sql += "                )";
		sql += "             -";
		sql += "             ROUND(CASE WHEN DECODE(NVL(MLB.\"AcctCode\", ' '), '990', '990', 'OTHER') = 'OTHER' ";
		sql += "                       AND I.\"YearMonth\" = :lastYearMonth";
		sql += "                 THEN NVL(I.\"AccumDPAmortized\", 0)";
		sql += "                 ELSE 0 END";
		sql += "                ))";
