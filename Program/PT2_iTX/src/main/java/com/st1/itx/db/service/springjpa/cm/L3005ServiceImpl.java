...
public class L3005ServiceImpl extends ASpringJpaParm implements InitializingBean {
	...
	public List<Map<String, String>> findAll(int index, int limit, TitaVo titaVo) throws LogicException {
		...
		sql += "   ,l.\"TmpAmt\"              ";
		sql += "   ,l.\"AcSeq\"               ";
		sql += "   ,ROW_NUMBER() OVER (Partition By l.\"CustNo\", l.\"TxKind\"              ";
		sql += "                                   ,CASE WHEN l.\"TxKind\" = 1 THEN l.\"AcDate\" ELSE 0 END ";
		sql += "                                   ,CASE WHEN l.\"TxKind\" = 1 THEN l.\"TitaTlrNo\" ELSE ' ' END ";
		sql += "                                   ,CASE WHEN l.\"TxKind\" = 1 THEN l.\"TitaTxtNo\" ELSE ' ' END ";
		...
		sql += "  FROM (              ";
		sql += "        SELECT              ";
		sql += "         CASE WHEN \"CreateEmpNo\" in ('999999') AND \"TitaTxCd\" in ('L3100','L3701') THEN 6               ";
		sql += "              WHEN \"TitaTxCd\" in ('L3721') THEN 5               ";
		sql += "       		  WHEN \"AcctCode\" IN ('TCK','RCK','BCK') THEN 4                ";
		sql += "       		  WHEN \"TempReasonCode\"  IN ('3','6','03','06') THEN 4              ";
		sql += "              WHEN \"AcctCode\" IN ('T11','T12','T13') THEN 3               ";
		sql += "              WHEN \"TmpAmt\" <> 0 THEN 2                ";
		sql += "              WHEN \"TxBorm\" > 0 THEN 1                 ";
		sql += "     		  ELSE 0              ";
		sql += "       	 END                        AS \"TxKind\"               ";
		sql += "        ,\"CustNo\"              ";
		sql += "        ,\"AcDate\"	                      ";
		...
		sql += "              ,MAX(\"CreateEmpNo\") AS \"CreateEmpNo\"              ";
		sql += "              ,MAX(CASE WHEN \"AcctCode\" IN ('TCK','RCK','BCK','T11','T12','T13') THEN \"AcctCode\"  ";
		sql += "                        ELSE '   ' END)  AS  \"AcctCode\"           ";
		sql += "       		  ,MAX(NVL(JSON_VALUE(\"OtherFields\", '$.TempReasonCode'),' '))  AS  \"TempReasonCode\"    ";
		sql += "              ,MAX(CASE WHEN \"BormNo\" > 0 THEN 1                  ";
		sql += "                        ELSE 0     END)  AS \"TxBorm\"              ";
