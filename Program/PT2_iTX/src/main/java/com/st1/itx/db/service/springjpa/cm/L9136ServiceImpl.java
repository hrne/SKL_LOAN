...
public class L9136ServiceImpl extends ASpringJpaParm implements InitializingBean {
	...
	public List<Map<String, String>> findAll(TitaVo titaVo) throws Exception {
		...
		sql += "		  ,NVL(TO_CHAR(Cl.\"ApproveNo\"),TO_CHAR(NVL(JSON_VALUE(TR.\"TranData\",'$.ApplNo'),'       '))) AS \"ApproveNo\"";
		...
		sql += "		  ,CASE WHEN T.\"Reason\" LIKE '%刪除%' THEN  '[\"' || TO_CHAR(T.\"Reason\") || '\"]' ELSE JSON_QUERY(T.\"Content\",'$[*].f' WITH WRAPPER) END AS \"Item\"";
...
//		sql += "						     AND TRIM(TR.\"MrKey\") = T.\"MrKey\"";
	...
	public List<Map<String, String>> findAll2(TitaVo titaVo) throws Exception {
		...
		sql += "	UNION";
		sql += "	SELECT T.\"Entdy\" AS \"AcDate\"";
		sql += "		  ,'00000000' AS \"TxSeq\"";
		sql += "		  ,T.\"CustNo\" AS \"CustNo\"";
		sql += "		  ,0 AS \"FacmNo\"";
		sql += "		  ,0 AS \"BormNo\"";
		sql += "		  ,SUBSTR(NVL(cm.\"CustName\",'     '),0,5) AS \"CustName\"";
		sql += "		  ,'       ' AS \"ApproveNo\"";
		sql += "		  ,'' AS \"ClCode1\"";
		sql += "		  ,'' AS \"ClCode2\"";
		sql += "		  ,' ' AS \"ClName\"";
		...
		sql += "		  ,' ' AS \"Old\"";
		sql += "		  ,'結清滿五年客戶' AS \"New\"";
		sql += "		  ,NVL(CE.\"Fullname\",' ') AS \"Name\"";
		sql += "		  ,NVL(CE2.\"Fullname\",' ') AS \"SupNoName\"";
		sql += "		  ,T.\"TranNo\" AS \"TranNo\"";
		sql += "		  ,TC.\"TranItem\" AS \"TranItem\"";
		sql += "		  ,2 AS \"Seq\"";
		sql += "		  ,T.\"TlrNo\" AS \"TlrNo\"";
		sql += "	FROM \"TxInquiry\" T";
		sql += "	LEFT JOIN \"CustMain\" CM ON CM.\"CustNo\" = T.\"CustNo\"";
