...
//				UnitChiefName	員工姓名
	...
	private void procHlThreeLaqhcp(TitaVo titaVo) throws LogicException {
		...
		if (data != null && data.size() > 0) {
			for (Map<String, String> d : data) {
				...
				if (d.get("UnitCode") == null && d.get("DistCode") == null) {
	...
	private void procHlAreaLnYg6Pt(TitaVo titaVo) throws LogicException {
		...
		sql += "left join (";
		sql += "  select bb.\"AreaCode\",sum(aa.\"PerfCnt\") as \"lPerfCnt\",sum(aa.\"PerfAmt\") as \"lPerfAmt\" ";
		...
		sql += "left join (";
		sql += "  select bb.\"AreaCode\",sum(aa.\"PerfCnt\") as \"PerfCnt\",sum(aa.\"PerfAmt\") as \"PerfAmt\" ";
		sql += "  from \"PfBsDetail\" aa ";
...
//				this.info(d.get("UnitCode") + "=" + d.get("UnitItem") + "/" + d.get("UnitItem").toString().length());
				...
				String unitItem = d.get("UnitItem");
				if (unitItem.length() > 10) {
					unitItem = unitItem.substring(0, 10);
				}
				tHlAreaData.setAreaName(unitItem);// 區域名稱--VARCHAR2(20)
