...
import com.st1.itx.db.service.springjpa.cm.L9138ServiceImpl;
import com.st1.itx.util.common.MakeReport;
import com.st1.itx.util.common.data.ReportVo;
import com.st1.itx.util.format.StringCut;

@Component("L9138Report")
@Scope("prototype")
public class L9138Report extends MakeReport {

	@Autowired
	L9138ServiceImpl L9138ServiceImpl;
	...
	public void printHeader() {
		...
		this.print(-3, newBorder.length() / 2, this.getRptItem(), "C");

		this.print(-5, newBorder.length() / 2, showRocDate(this.getReportDate()), "C");
	...
	private void fillData(TitaVo titaVo,int acdate) {

		this.newPage();

		List<Map<String, String>> lL9138 = null;

		try {
			lL9138 = L9138ServiceImpl.findAll(titaVo,acdate);
		} catch (Exception e) {
			StringWriter errors = new StringWriter();
			e.printStackTrace(new PrintWriter(errors));
			this.error("L9138ServiceImpl.findAll error = " + errors.toString());
		}
		...
		if (lL9138 != null && lL9138.size() != 0) {
			...
			for (Map<String, String> tLDVo : lL9138) {
				print(1, 0, "    ");
	...
	public boolean exec(TitaVo titaVo,int acdate) throws LogicException {
		this.info("L9138Report exec ...");
