...
//				3.於original檔無資料者
			...
			try {
				// *** 折返控制相關 ***
				resultList = l4962ServiceImpl.findAll(0, Integer.MAX_VALUE, titaVo);
			} catch (Exception e) {
		...
		if (!"N".equals(CommericalFlag)) {
			...
			sInsuRenew = insuRenewService.findL4962A(iInsuEndMonthFrom1, iInsuEndMonthTo1, 0, Integer.MAX_VALUE,
					titaVo);
