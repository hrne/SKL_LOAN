...
//		抓取媒體日為今日者
		...
		if (slEmpDeductSchedule == null) {
			...
			for (EmpDeductSchedule tEmpDeductSchedule : slEmpDeductSchedule.getContent()) {
				...
				if ("4".equals(tEmpDeductSchedule.getAgType1()) || "5".equals(tEmpDeductSchedule.getAgType1())) {
					if (iOpItem == 1) {
						procCode = tEmpDeductSchedule.getAgType1();
					}
				} else {
					if (iOpItem == 2) {
						procCode = tEmpDeductSchedule.getAgType1();
				...
				if (!procCode.isEmpty()) {
					deleteEmpDeductDtl(tEmpDeductSchedule.getEntryDate(), procCode, titaVo);
					fg = true;
				}
			}
		}
	...
	public void deleteEmpDeductDtl(int entryDate, String procCode, TitaVo titaVo) throws LogicException {
		ArrayList<String> procCodeList = new ArrayList<>();
		procCodeList.add(procCode);
		Slice<EmpDeductDtl> slEmpDeductDtl = empDeductDtlService.entryDateRng(entryDate + 19110000,
				entryDate + 19110000, procCodeList, 0, Integer.MAX_VALUE, titaVo);
