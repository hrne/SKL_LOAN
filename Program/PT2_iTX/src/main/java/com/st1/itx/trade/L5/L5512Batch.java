...
public class L5512Batch extends TradeBuffer {
	...
	private void toCheck(TitaVo titaVo) throws LogicException {
		...
		// 刪除本月保費檢核資料(重新執行用)
		Slice<PfInsCheck> slPfInsCheck = pfInsCheckService.findCheckWorkMonthEq(iWorkMonth, 2, 0, Integer.MAX_VALUE,
				titaVo);
...
//		webClient.sendPost(dateUtil.getNowStringBc(), "2300", titaVo.getTlrNo(), "", "", "", "介紹人加碼獎勵津貼發放檔已產生，業績筆數=" + lPfPlus.size() + ", 追回業績筆數=" + lPfMinus.size(), titaVo);
	...
	private void calculate(int iCustNo, int iFacmNo, ArrayList<PfReward> lPfFac, TitaVo titaVo) throws LogicException {
		...
		for (PfReward iPf : lPfFac) {
			...
			if (lPfPlus.size() >= 0) {
				if (iPf.getWorkMonth() == iWorkMonth) {
					tPfInsCheck = pfCheckInsuranceCom.check(2, iCustNo, iFacmNo, iWorkMonth, iWorkMonth, titaVo); // 2.介紹人加碼獎勵津貼
			...
			if (addBonusLM.compareTo(BigDecimal.ZERO) > 0) {
				if (iPf.getWorkMonth() < iWorkMonth && iPf.getIntroducerAddBonus().compareTo(BigDecimal.ZERO) > 0) {
					tPfInsCheck = pfCheckInsuranceCom.check(2, iCustNo, iFacmNo, iWorkMonth, iWorkMonth, titaVo); // 2.介紹人加碼獎勵津貼
...
//				}
	...
	private int makeMedia(TitaVo titaVo) throws LogicException {
		...
		makeFile.open(titaVo, titaVo.getEntDyI(), titaVo.getKinbr(), "L5512.2", "介紹加碼獎金媒體檔", "TOTAL.csv", 2);
		int cnt = 0;
