...
@Table(name = "`LoanIfrsAp`")
public class LoanIfrsAp implements Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = -6021945400776825249L;

	@EmbeddedId
	private LoanIfrsApId loanIfrsApId;
	...
	// 法拍及火險費用
	/*
	 * 法務費: "MonthlyFacBal"."LawFee" (依照戶號+額度加總法務費，並按餘額比例分配至每一筆) +火險費:
	 * "MonthlyFacBal"."FireFee" (依照戶號+額度加總火險費，並按餘額比例分配至每一筆)
	 */
	@Column(name = "`Fee`")
	...
	// 核准利率
	/*
	 * 至小數點後第6位。例如，利率為2.12%，則本欄位值表示0.021200。契約是階梯式...etc,抓取第一年的合約利率(不管加碼利率)(ex:第一年1.
	 * 4%，第二年1.5%，則本欄位填入1.4%)
	 */
	...
	// 契約當時還款方式
	/* 截至月底日當時之還款方式1=按期繳息(到期還本)；2=平均攤還本息；3=平均攤還本金；4=到期繳息還本 */
	@Column(name = "`AmortizedCode`", length = 1)
	private String amortizedCode;

	// 契約當時利率調整方式
	/* 截至月底日當時之利率調整方式1=機動；2=固定；3=固定階梯；4=浮動階梯 */
	...
	// 擔保品地區別
	/*
	 * A=臺北市B=新北市C=桃園市D=台中市E=台南市F=高雄市G=其他(基隆市、新竹縣、新竹市、苗栗縣、彰化縣、南投縣、雲林縣、嘉義縣、嘉義市、屏東縣、
	 * 宜蘭縣、花蓮縣、臺東縣、金門縣、澎湖縣、連江縣、南海島、釣魚臺
	 */
	...
	public LoanIfrsApId getLoanIfrsApId() {
		return this.loanIfrsApId;
	}

	public void setLoanIfrsApId(LoanIfrsApId loanIfrsApId) {
		this.loanIfrsApId = loanIfrsApId;
	}
	...
	/**
	 * 利率(撥款)<br>
	 * 抓取月底時適用利率 至小數點後第6位。例如，利率為2.1234%，則本欄位值表示0.021234 (台幣放款無法蒐集此欄位則以空白表示)
	 * 
	 * @return BigDecimal
	 */
	...
	/**
	 * 契約當時還款方式<br>
	 * 截至月底日當時之還款方式 1=按期繳息(到期還本)； 2=平均攤還本息； 3=平均攤還本金； 4=到期繳息還本
	 * 
	 * @return String
	 */
	...
	/**
	 * 契約當時利率調整方式<br>
	 * 截至月底日當時之利率調整方式 1=機動； 2=固定； 3=固定階梯； 4=浮動階梯
	 * 
	 * @return String
	 */
	...
	/**
	 * 契約約定當時還本週期<br>
	 * 截至月底日當時之還本週期 若為到期還本，則填入0； 若按月還本，則填入1； 季繳，3； 半年，6； 年繳,12。
	 * 
	 * @return Integer
	 */
	...
	/**
	 * 契約約定當時繳息週期<br>
	 * 截至月底日當時之繳息週期 若為到期繳息，則填入0； 若按月還本，則填入1； 季繳，3； 半年，6； 年繳,12。
	 * 
	 * @return Integer
	 */
	...
	/**
	 * 擔保品地區別<br>
	 * A=臺北市 B=新北市 C=桃園市 D=台中市 E=台南市 F=高雄市
	...
	/**
	 * 該筆額度是否可循環動用<br>
	 * 0: 非循環動用 1: 循環動用 若註記為1，且本金餘額＜核准金額，但是可動用餘額=0；需確認是否實為循環額度
	 * 
	 * @return String
	 */
	...
	public String toString() {
		return "LoanIfrsAp [loanIfrsApId=" + loanIfrsApId + ", custId=" + custId + ", applNo=" + applNo + ", acCode=" + acCode + ", status=" + status + ", firstDrawdownDate=" + firstDrawdownDate
				+ ", drawdownDate=" + drawdownDate + ", facLineDate=" + facLineDate + ", maturityDate=" + maturityDate + ", lineAmt=" + lineAmt + ", drawdownAmt=" + drawdownAmt + ", acctFee="
				+ acctFee + ", loanBal=" + loanBal + ", intAmt=" + intAmt + ", fee=" + fee + ", rate=" + rate + ", ovduDays=" + ovduDays + ", ovduDate=" + ovduDate + ", badDebtDate=" + badDebtDate
				+ ", badDebtAmt=" + badDebtAmt + ", gracePeriod=" + gracePeriod + ", approveRate=" + approveRate + ", amortizedCode=" + amortizedCode + ", rateCode=" + rateCode + ", repayFreq="
				+ repayFreq + ", payIntFreq=" + payIntFreq + ", industryCode=" + industryCode + ", clTypeJCIC=" + clTypeJCIC + ", cityCode=" + cityCode + ", prodNo=" + prodNo + ", custKind="
				+ custKind + ", assetClass=" + assetClass + ", ifrsProdCode=" + ifrsProdCode + ", evaAmt=" + evaAmt + ", firstDueDate=" + firstDueDate + ", totalPeriod=" + totalPeriod + ", avblBal="
				+ avblBal + ", recycleCode=" + recycleCode + ", irrevocableFlag=" + irrevocableFlag + ", tempAmt=" + tempAmt + ", acCurcd=" + acCurcd + ", acBookCode=" + acBookCode + ", currencyCode="
				+ currencyCode + ", exchangeRate=" + exchangeRate + ", lineAmtCurr=" + lineAmtCurr + ", drawdownAmtCurr=" + drawdownAmtCurr + ", acctFeeCurr=" + acctFeeCurr + ", loanBalCurr="
				+ loanBalCurr + ", intAmtCurr=" + intAmtCurr + ", feeCurr=" + feeCurr + ", avblBalCurr=" + avblBalCurr + ", tempAmtCurr=" + tempAmtCurr + ", createDate=" + createDate
