...
//      SSL
		 ...
		 * ssl.setKeystorePassword("password"); ssl.setSslProtocol("SSL"); // set the
...
//      解決辦法：將文件覆制到指定目錄(本文指定到根目錄)下然後FTPServer才能讀取到。
		 ...
		 * PropertiesUserManagerFactory(); String tempPath =
		 * System.getProperty("java.io.tmpdir") + System.currentTimeMillis() +
		 * ".properties"; File tempConfig = new File(itxResource + "users.properties");
		 * // ClassPathResource resource = new
		 * ClassPathResource("spring/ftp/users.properties"); //
...
//      6.2、基於數據庫來存儲用戶實例
		...
		dbUserManagerFactory.setSqlUserInsert("INSERT INTO FTP_USER (userid, userpassword, homedirectory, " + "enableflag, writepermission, idletime, uploadrate, downloadrate) VALUES "
