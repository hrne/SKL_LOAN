...
@Component("L4212Report")
@Scope("prototype")

public class L4212Report extends MakeReport {
...
//	每頁筆數
	...
	public void printHeaderP() {
		...
		this.print(-2, 3, "程式ID：" + "L4212Report");
		this.print(-2, 80, "新光人壽保險股份有限公司", "C");
		String tim = String.valueOf(Integer.parseInt(dateUtil.getNowStringBc().substring(2, 4)));
//		月/日/年(西元後兩碼)
		this.print(-2, 155, "日    期：" + dateUtil.getNowStringBc().substring(4, 6) + "/"
				+ dateUtil.getNowStringBc().substring(6, 8) + "/" + tim, "R");
		this.print(-3, 3, "報　表：" + "L4212Report");
		this.print(-3, 80, "支票兌現總傳票明細表", "C");
		this.print(-3, 155, "時    間：" + dateUtil.getNowStringTime().substring(0, 2) + ":"
...
//		this.print(-6, 18,"流程別：" + ProcCode);
		...
		this.print(-9, 1,
				"  戶號    計息起日   計息迄日    姓名          扣款金額         本金         利息       違約金      暫收借      暫收貸      短繳   帳管費及其他   ");
		this.print(-10, 0,
				"---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------");
	}
...
//		設定每筆分頁的資料筆數 預設500筆 總長不可超過六萬
		...
		this.info("L4212Report exec");

		ReportVo reportVo = ReportVo.builder().setBrno(titaVo.getBrno()).setRptDate(titaVo.getEntDyI())
				.setRptCode("L4212").setRptItem("支票兌現總傳票明細表").setRptSize("A4").setSecurity("").setPageOrientation("L")
				.build();
...
//			1.每筆先印出明細
				...
				if (parse.stringToInteger(fnAllList.get(i).get("IntStartDate")) != 0) {
					this.print(0, 11,
							String.valueOf(parse.stringToInteger(fnAllList.get(i).get("IntStartDate")) - 19110000)); // 計息起日
				}

				if (parse.stringToInteger(fnAllList.get(i).get("IntEndDate")) != 0) {
					this.print(0, 21,
							String.valueOf(parse.stringToInteger(fnAllList.get(i).get("IntEndDate")) - 19110000)); // 計息迄日
				}
