...
public class LB201Report extends MakeReport {
	...
	private void genFile(TitaVo titaVo, List<Map<String, String>> LBList) throws LogicException {
		...
		// 首筆
		strContent = "JCIC-DAT-B201-V01-458" + StringUtils.repeat(" ", 5) + strToday + sfileNo2 + StringUtils.repeat(" ", 10) + makeFile.fillStringR("審查聯絡人－" + jcicEmpName, 20, ' ')
				+ makeFile.fillStringR(jcicEmpTel, 16, ' ') + makeFile.fillStringR("", 20, ' ') + makeFile.fillStringR("", 16, ' ') + StringUtils.repeat(" ", 80) + StringUtils.repeat(" ", 315);
...
//					}
	...
	private void genExcel(TitaVo titaVo, List<Map<String, String>> LBList) throws LogicException {
		...
		inf = "總行代號(1~3),分行代號(4~7),交易代碼(8),帳號屬性註記(9),本筆撥款帳號(10~59)," + "金額合計(60~69),授信戶IDN/BAN(70~79),上欄IDN或BAN錯誤註記(80),負責人IDN/負責之事業體BAN(81~90),上欄IDN或BAN錯誤註記(91),"
				...
				+ "擔保品類別(357~358),國內或國際聯貸(359),聯貸合約訂定日期(360~367),聯貸參貸比例(368~371),購地貸款註記(372),約定動工之一定期間(373~374)," + "實際興建年月(375~379),代放款註記(380),債務協商註記(381),空白(382),共同債務人或債務關係人身份代號1(383),"
				...
				+ "呆帳轉銷年月(473~477),聯貸主辦(管理)行註記(478~482),破產宣告日(或法院裁定開始清算日)(483~489),建築貸款註記(490)," + "授信餘額列報1（千元）之原始金額（元）(491~494),補充揭露案件註記－案件屬性(495),補充揭露案件註記－案件情形(496~497),空白(498~506),資料所屬年月(507~511),"
				+ "資料結束註記(512)";
