...
public class LC005ServiceImpl extends ASpringJpaParm implements InitializingBean {
	...
	public List<Map<String, String>> findAll(TitaVo titaVo, int index, int limit) throws Exception {
		...
		sql += "   and A.\"BrNo\"=:brno";
		sql += "   and A.\"FlowMode\"= 3 "; // 流程模式 1.待放行 2.待審核 3.待提交 9.已結案
		sql += "   and A.\"FlowStep\" in (1) "; // 流程步驟 1.登錄 2.放行 3.審核 4.審核放行
		sql += "   and A.\"FlowGroupNo\"=F.\"GroupNo\" "; // 流程科組別
		sql += "   and NVL(E.\"Entdy\", 0) > 0 "; // join到TxRecord
		sql += "   and E.\"TlrNo\"=:tlrNo"; // 登錄提交經辦為自己
		if (!"".equals(iTranNo)) {
		...
		this.info("LC005ServiceImpl sql=" + sql);
		Query query;
