...
public class L8314 extends TradeBuffer {
	...
	public ArrayList<TotaVo> run(TitaVo titaVo) throws LogicException {
		...
		if (!"4".equals(iTranKey_Tmp)) {
			...
			// 4
			// 檢核第8欄「是否同意報送例外處理檔案格式」填報'Y'者，方可補報送'42'或'43'檔案格式，否則予以剔退處理.-->檢核做在L8303和L8304最後extra項

			// 5 檢核第9，10欄「同意補報送資料檔案格式之資料別」僅限於"42", "43", "61"，其餘予以剔退處理.
			if (("Y".equals(iAgreeSend)) && (!Arrays.stream(acceptDataCode).anyMatch(iAgreeSendData1::equals)
					|| !Arrays.stream(acceptDataCode).anyMatch(iAgreeSendData2::equals))) {
				throw new LogicException("E0005", "「「同意補報送資料檔案格式資料別」僅限於'42','43','61'.");
			}
			// 5 end
