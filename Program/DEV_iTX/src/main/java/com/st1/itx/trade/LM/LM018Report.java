...
public class LM018Report extends MakeReport {
	// private static final Logger logger = LoggerFactory.getLogger(LM018Report.class);
	...
	public void printHeaderL() {
		...
		// I think it's MakeReport's problem, doesn't handle print pos well when fontsize is changed.
		// all those numbers are tuned by eyes, but I think it's around +6 offset globally.
		this.setFontSize(5);
	...
	public void exec(TitaVo titaVo) throws LogicException {
		...
		Subject[] subjects = new Subject[] { 
				Subject.Init("AA", -17, false), 
				Subject.Init("IA", -20, true), 
				Subject.Init("IB", -23, true),
				Subject.Init("IC", -26, true), 
				Subject.Init("ID", -29, true), 
				Subject.Init("IF", -32, true), 
				Subject.Init("IH", -35, true),
				Subject.Init("ZZ", -38, true) };
		...
		if (LM018List != null && LM018List.size() != 0) {
			...
			for (Map<String, String> LM018Vo : LM018List) {
				...
				if (!lastYearMonth.equals(LM018Vo.get("F1")))
				{
					...
					if (lastYearMonth != "")
					{
						this.print(-27, xPivot + xShift + 18, this.GetGroupIntTotal(subjects).divide(new BigDecimal(100000000), 3, BigDecimal.ROUND_UP).toString(), "C");
					
						// Output: year-to-be-ended sums
               			this.print(-41, xPivot + xShift + 4, this.GetBalTotal(subjects).divide(new BigDecimal(100000000), 3, BigDecimal.ROUND_UP).toString(), "C");
						this.print(-41, xPivot + xShift + 20, this.GetIntTotal(subjects).divide(new BigDecimal(100000000), 3, BigDecimal.ROUND_UP).toString(), "C");
                                        }
				...
				if (thisSubject != null) {
					...
					{
						this.print(thisSubject.printY, xPivot + xShift - 6, f3.divide(new BigDecimal(100000000), 3, BigDecimal.ROUND_UP).toString(), "C");
					}
            ...
            this.print(-27, xPivot + xShift - 6, this.GetGroupIntTotal(subjects).divide(new BigDecimal(100000000), 3, BigDecimal.ROUND_UP).toString(), "C");
			...
			this.print(-41, xPivot + xShift - 6, this.GetIntTotal(subjects).divide(new BigDecimal(100000000), 3, BigDecimal.ROUND_UP).toString(), "C");
