 ...
 * @version 1.0.0
 */
public class BS441 extends TradeBuffer {
	// private static final Logger logger = LoggerFactory.getLogger(BS441.class);
...
//	寄送筆數
	...
	ArrayList<OccursList> doReport = new ArrayList<>();

	@Override
	public ArrayList<TotaVo> run(TitaVo titaVo) throws LogicException {
		this.info("active BS441 ");
...
//		設定每筆分頁的資料筆數 預設500筆 總長不可超過六萬
		...
		if (titaVo.isHcodeNormal()) {
			...
			if (checkFlag) {
				if (doReport != null && doReport.size() > 0) {
					sendMsg = "銀扣媒體檔未產出清單、媒體檔已完成。";
				} else {
					sendMsg = "未產出筆數=0。媒體檔已完成。";
				}
			}
		}
		...
		if (checkFlag) {
			...
			webClient.sendPost(dateUtil.getNowStringBc(), "2300", titaVo.getTlrNo(), "Y", "L4452", titaVo.getTlrNo(),
					sendMsg, titaVo);
		}
	...
	private void hcodeNormal(TitaVo titaVo) throws LogicException {
...
//		this.totaVo.putParam("OFormFlag", 1);
		...
		sBankDeductDtl = bankDeductDtlService.entryDateRng(entryDate, entryDate, this.index, this.limit);
...
//			寫入媒體檔
			ArrayList<BankDeductDtl> lMediaBankDeductDtl = new ArrayList<BankDeductDtl>();
			lMediaBankDeductDtl = sBankDeductDtl == null ? null
					: new ArrayList<BankDeductDtl>(sBankDeductDtl.getContent());

			if (lMediaBankDeductDtl != null && lMediaBankDeductDtl.size() != 0) {
//				post
				setPostDeductMedia(lMediaBankDeductDtl, titaVo);
//				ACH
				setAchDeductMedia(lMediaBankDeductDtl, titaVo);
...
//				ACH
				...
				l4452Report.doReport(doReport, titaVo);
			}
		} else {
	...
	private void hcodeErase(TitaVo titaVo) throws LogicException {
//		刪除今日於媒體檔之資料
		deleteOldPostMedia(mediaDate);
		deleteOldAchMedia(mediaDate, "1");
		deleteOldAchMedia(mediaDate, "2");

		List<BankDeductDtl> lBankDeductDtl = new ArrayList<BankDeductDtl>();

		Slice<BankDeductDtl> sBankDeductDtl = null;

		sBankDeductDtl = bankDeductDtlService.entryDateRng(entryDate, entryDate, this.index, this.limit, titaVo);

		lBankDeductDtl = sBankDeductDtl == null ? null : sBankDeductDtl.getContent();

		if (lBankDeductDtl != null && lBankDeductDtl.size() != 0) {
			for (BankDeductDtl tBankDeductDtl : lBankDeductDtl) {
				updateBankDeductDtl(tBankDeductDtl, 0, "", 0, "");
			}
		}
...
//					欄位不夠長，順序為帳號、AML、扣帳金額為零
					...
					updateBankDeductDtl(tBankDeductDtl, 0, "3", 0, mediaCode);
					this.info("檢核未過跳過...");
					continue;
				} else {
					mediaCode = "Y";
...
//					重複的撥款seq不+1
				...
				updateBankDeductDtl(tBankDeductDtl, mediaDate, "3", mediaSeq, mediaCode);
...
//					入帳扣款別(MBKTRX)】=1火險、2期款、3帳管、4契變手續費
				...
				tPostDeductMedia.setTransDate(entryDate);
				tPostDeductMedia.setRepayAcctNo(tBankDeductDtl.getRepayAcctNo());
//					原 扣款人ID+郵局存款別(POSCDE)+戶號) 右補空
				tPostDeductMedia.setPostUserNo(FormatUtil.padLeft(FormatUtil.padX(tCustMain.getCustId(), 10)
						+ tBankDeductDtl.getPostCode() + FormatUtil.pad9("" + tBankDeductDtl.getCustNo(), 7), 20));
//					扣款人ID+郵局存款別(POSCDE)+戶號) 預計補2位帳號碼
//					if (tBankDeductDtl.getRepayAcctSeq() != null) {
//						repayAcctSeq = tBankDeductDtl.getRepayAcctSeq();
//					} else {
//						repayAcctSeq = " ";
//					}
//					tPostDeductMedia.setPostUserNo(FormatUtil.padLeft(FormatUtil.padX(tCustMain.getCustId(), 10)
//							+ tBankDeductDtl.getPassBookCode() + FormatUtil.pad9("" + tBankDeductDtl.getCustNo(), 7)
//							+ FormatUtil.pad9(repayAcctSeq.trim(), 2), 20));
//					預計為計息迄日+額度編號+入帳扣款別
		...
		if (mediaSeq > 1) {
			try {
				postDeductMediaService.insertAll(lPostDeductMedia);
				this.info("L4452 PostDeductMedia insert Success");
			} catch (DBException e) {
...
//					欄位不夠長，順序為帳號、AML、扣帳金額為零
					...
					if ("103".equals(tBankDeductDtl.getRepayBank())) {
						updateBankDeductDtl(tBankDeductDtl, 0, "1", 0, mediaCode);
					} else {
						updateBankDeductDtl(tBankDeductDtl, 0, "2", 0, mediaCode);
					}
					this.info("不產出，僅寫入報表...");
					continue;
				} else {
...
//					重複的撥款seq不+1
				...
				if ("103".equals(tBankDeductDtl.getRepayBank())) {
					updateBankDeductDtl(tBankDeductDtl, mediaDate, "1", mediaSeq1, mediaCode);
				} else {
					updateBankDeductDtl(tBankDeductDtl, mediaDate, "2", mediaSeq2, mediaCode);
				}
...
//					tAchDeductMedia.setDepCode(depCode);
		...
		if (mediaSeq1 >= 1 || mediaSeq2 >= 1) {
			try {
				achDeductMediaService.insertAll(lAchDeductMedia);
				this.info("L4452 AchDeductMedia insert Success");
			} catch (DBException e) {
				throw new LogicException("E0005", "L4452 資料重複，請先訂正。");
			}
		}
...
//		none
		...
		sPostDeductMedia = postDeductMediaService.mediaDateEq(mediaDate, this.index, this.limit);
...
//				含兩位小數位
				...
				occursList.putParam("OccPrtCustNo", "1");
...
//		AHP12P_扣出.txt 他行  "‪D:\\temp\\AHP11P_扣出.txt";
		...
		sAchDeductMedia = achDeductMediaService.mediaDateEq(mediaDate, "1", this.index, this.limit);
...
//			String outputFilePath11 = outFolder + "AHP11P_扣出.txt";
		...
		s2AchDeductMedia = achDeductMediaService.mediaDateEq(mediaDate, "2", this.index, this.limit);
...
//				戶號7+額度3+入帳扣款別1+繳息迄日8 左靠右補空白
	...
	private void deleteOldPostMedia(int today) throws LogicException {
		List<PostDeductMedia> lPostDeductMedia = new ArrayList<PostDeductMedia>();

		Slice<PostDeductMedia> sPostDeductMedia = null;

		sPostDeductMedia = postDeductMediaService.mediaDateEq(today, this.index, this.limit);
		...
		if (lPostDeductMedia != null && lPostDeductMedia.size() != 0) {
			...
			for (PostDeductMedia tPostDeductMedia : lPostDeductMedia) {
				...
				postDeductMediaService.holdById(tPostDeductMedia);
				try {
					postDeductMediaService.delete(tPostDeductMedia);
	...
	private void deleteOldAchMedia(int today, String mediaKind) throws LogicException {
		List<AchDeductMedia> lAchDeductMedia = new ArrayList<AchDeductMedia>();

		Slice<AchDeductMedia> sAchDeductMedia = null;

		sAchDeductMedia = achDeductMediaService.mediaDateEq(today, mediaKind, this.index, this.limit);
		...
		if (lAchDeductMedia != null && lAchDeductMedia.size() != 0) {
			...
			for (AchDeductMedia tAchDeductMedia : lAchDeductMedia) {
				...
				achDeductMediaService.holdById(tAchDeductMedia);
				try {
					achDeductMediaService.delete(tAchDeductMedia);
	...
	private void updateBankDeductDtl(BankDeductDtl t2BankDeductDtl, int mediaDate, String mediaKind, int mediaSeq,
			String mediaCode) throws LogicException {
		this.info("updateBankDeductDtl Start...");

		BankDeductDtl tBankDeductDtl = new BankDeductDtl();

		tBankDeductDtl = bankDeductDtlService.holdById(t2BankDeductDtl.getBankDeductDtlId());
		...
		try {
			bankDeductDtlService.update(tBankDeductDtl);
		} catch (DBException e) {
...
//						欄位不夠長，順序為帳號、AML、扣帳金額為零
	...
	private void doReport(String reason, String confirmCode, BankDeductDtl tBankDeductDtl, TitaVo titaVo) {
		...
		doReport.add(occursList);
	}

	private void doReport(String reason, String confirmCode, PostDeductMedia tPostDeductMedia, TitaVo titaVo) {
		this.info("doReport...");
		this.info("ConfirmCode : " + confirmCode);
		this.info("RepayType : " + tPostDeductMedia.getRepayType());

		OccursList occursList = new OccursList();
		String repayType = "";
		String note = "";

		CdCode t2CdCode = cdCodeService.getItemFirst(4, "RepayTypeSearch",
				FormatUtil.pad9("" + tPostDeductMedia.getRepayType(), 2), titaVo);
		...
		occursList.putParam("OOCustNo", tPostDeductMedia.getCustNo());
		occursList.putParam("OOFacmNo", tPostDeductMedia.getFacmNo());
		occursList.putParam("OOBormNo", "");
		occursList.putParam("OORepayType", repayType);
		occursList.putParam("OORepayAmt", tPostDeductMedia.getRepayAmt());
		occursList.putParam("OONote", note);
		doReport.add(occursList);
	}

	private void doReport(String reason, String confirmCode, AchDeductMedia tAchDeductMedia, TitaVo titaVo) {
		this.info("doReport...");
		this.info("ConfirmCode : " + confirmCode);
		this.info("RepayType : " + tAchDeductMedia.getRepayType());
		...
		CdCode t2CdCode = cdCodeService.getItemFirst(4, "RepayTypeSearch",
				FormatUtil.pad9("" + tAchDeductMedia.getRepayType(), 2), titaVo);
		...
		occursList.putParam("OOCustNo", tAchDeductMedia.getCustNo());
		occursList.putParam("OOFacmNo", tAchDeductMedia.getFacmNo());
		occursList.putParam("OOBormNo", "");
		occursList.putParam("OORepayType", repayType);
		occursList.putParam("OORepayAmt", tAchDeductMedia.getRepayAmt());
		occursList.putParam("OONote", note);
		doReport.add(occursList);
	}
...
//	暫時紀錄戶號額度
	private class tmpFacm {
		...
		private BS441 getEnclosingInstance() {
			return BS441.this;
		}
	}
}