...
public class LM001Report extends MakeReport {
	// private static final Logger logger = LoggerFactory.getLogger(LM001Report.class);
	...
	public void exec(TitaVo titaVo) throws LogicException {
		...
		int bsEntDy = titaVo.getEntDyI() + 19110000; // 西園帳務日
		...
		int thisMonth = bsEntDy / 100;

		int lastMonth = lastMonthEndDay / 100;
		...
		if (listLM001 != null && listLM001.size() > 0) {
			...
			BigDecimal thisMonthCnt = new BigDecimal(tLM001.get("F0")); // 專案運用-期末有效戶數
			BigDecimal thisMonthLoanBalTotal = new BigDecimal(tLM001.get("F1")); // 專案運用-期末有效金額
			BigDecimal lastMonthCnt = new BigDecimal(tLM001.get("F2")); // 專案運用-期初有效戶數
			BigDecimal lastMonthLoanBalTotal = new BigDecimal(tLM001.get("F3")); // 專案運用-期初有效金額
			BigDecimal thisMonthAddCnt = new BigDecimal(tLM001.get("F4")); // 專案運用-本期增加戶數
			BigDecimal thisMonthAddLoanBalTotal = new BigDecimal(tLM001.get("F5")); // 專案運用-本期增加有效金額
		...
		print(1, 1, "　　　　║　　　　　　　　　　　║　　　　　　　　　　　　║　　　　　　　　　　　　║　　　　　　　　　　　　║");
		// 表二
		if (listLM001Rate != null && listLM001Rate.size() > 0) {

			Map<String, String> tLM001Rate = listLM001Rate.get(0);

			BigDecimal firstRateMinimum = new BigDecimal(tLM001Rate.get("F0"));
			...
			BigDecimal firstRateMaximum = new BigDecimal(tLM001Rate.get("F1"));
			...
			BigDecimal continueRateMinimum = new BigDecimal(tLM001Rate.get("F2"));
			...
			BigDecimal continueRateMaximum = new BigDecimal(tLM001Rate.get("F3"));
		...
		if(higherRateList != null && higherRateList.size() != 0) {
	...
	private void printSheetHeader(String sheet) throws LogicException {
		...
		makeExcel.setValue(1, 2, dateUtil.getNowStringTime().substring(0, 2) + ":"
				+ dateUtil.getNowStringTime().substring(2, 4) + ":" + dateUtil.getNowStringTime().substring(4, 6), "R", headerStyleVo);
