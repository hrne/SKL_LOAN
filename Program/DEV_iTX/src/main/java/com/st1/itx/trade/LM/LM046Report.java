...
public class LM046Report extends MakeReport {
	// private static final Logger logger = LoggerFactory.getLogger(LM046Report.class);
	...
	private void exportExcel(TitaVo titaVo, List<Map<String, String>> LDList) throws LogicException {
		...
		for (Map<String, String> tLDVo : LDList)
		{
			...
			makeExcel.setValue(4 + count, 6, safelyDivide(tLDVo.get("F10"), 1000, 0).add(safelyDivide(tLDVo.get("F12"), 1000, 0))
					.add(safelyDivide(tLDVo.get("F4"), 1000, 0)).add(safelyDivide(tLDVo.get("F6"), 1000, 0)), "#,##0");
			makeExcel.setValue(4 + count, 9, safelyDivide(tLDVo.get("F3"), 1, 0).add(safelyDivide(tLDVo.get("F5"), 1, 0))
					.add(safelyDivide(tLDVo.get("F9"), 1, 0).add(safelyDivide(tLDVo.get("F11"), 1, 0))), "#,##0");
			makeExcel.setValue(4 + count, 11, safelyDivide(
					safelyDivide(tLDVo.get("F10"), 1, 5).add(safelyDivide(tLDVo.get("F12"), 1, 5))
					.add(safelyDivide(tLDVo.get("F4"), 1, 5)).add(safelyDivide(tLDVo.get("F6"), 1, 5)),
					safelyDivide(tLDVo.get("F2"), 1, 5).add(safelyDivide(tLDVo.get("F8"), 1, 0)), 5), "0.00%");
			...
			totals[2] = new BigDecimal(tLDVo.get("F3")).add(new BigDecimal(tLDVo.get("F8")));
			totals[3] = safelyDivide(tLDVo.get("F10"), 1000, 0);
			totals[4] = new BigDecimal(tLDVo.get("F11"));
			totals[5] = safelyDivide(tLDVo.get("F12"), 1000, 0);
