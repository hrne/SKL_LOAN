...
public class LM043Report extends MakeReport {
	// private static final Logger logger = LoggerFactory.getLogger(LM043Report.class);
	...
	public void exec(TitaVo titaVo) throws LogicException {
		...
		makeExcel.setValue(1, 1, entdy.substring(0, entdy.length() - 2) + "." + entdy.substring(entdy.length() - 2, entdy.length()));
	...
	private void exportExcel(TitaVo titaVo, List<Map<String, String>> LDList) throws LogicException {
		...
		for (Map<String, String> tLDVo : LDList) {
			...
			for (int i = 0; i < tLDVo.size(); i++) {
				...
				switch(i) {
				case 2:
					BigDecimal cnt = tLDVo.get(ad) == null ? BigDecimal.ZERO : new BigDecimal(tLDVo.get(ad));
					if(type != 0) {
					...
					break;
				case 3:
					BigDecimal prinBalance = tLDVo.get(ad) == null ? BigDecimal.ZERO : new BigDecimal(tLDVo.get(ad));
					if(type == 0) {
					...
					break;
				default:
					makeExcel.setValue(row, col + i, tLDVo.get(ad), "R");
				}
			} // for
			row++;
		} // for
