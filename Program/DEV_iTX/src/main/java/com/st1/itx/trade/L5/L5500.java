...
public class L5500 extends TradeBuffer {
	...
	public ArrayList<TotaVo> run(TitaVo titaVo) throws LogicException {
		...
		/*
		 * String WorkDate=titaVo.getParam("WorkDate").trim();//工作日 int PerfDate=0;
		 * if(WorkDate!=null && WorkDate.length()!=0) {
		 * PerfDate=parse.stringToInteger(WorkDate); if(PerfDate>0 &&
		 * String.valueOf(PerfDate).length()==7) { PerfDate=PerfDate+19110000; } }else {
		 * //工作日未填寫 throw new LogicException(titaVo, "XXXX", "[工作日]未填寫"); }
		 */
		// 下一業績日期
