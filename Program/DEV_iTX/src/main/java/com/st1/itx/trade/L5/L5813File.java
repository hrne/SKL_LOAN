...
public class L5813File extends MakeFile {
	...
	public void doFile(TitaVo titaVo) throws LogicException {
		...
		if(sYearlyHouseLoanInt==null) {
			...
			for(YearlyHouseLoanInt iYearlyHouseLoanInt : sYearlyHouseLoanInt) {
				...
				if(tClFac != null) {
					...
					if(tClBuilding!=null) {
						...
						bdContractDate = tClBuilding.getHouseBuyDate();//房屋所有權取得日
					}
			...
			strField += makeFile.fillStringL(String.valueOf(iFirstDrawdownDate), 7, '0'); // 貸款起日 7碼
			strField += makeFile.fillStringL(String.valueOf(iMaturityDate), 7, '0'); // 貸款迄日 7碼
			strField += makeFile.fillStringL(String.valueOf(iLoanBal), 10, '0'); // 截至本年度未償還本金餘額 10碼
			strField += makeFile.fillStringL(iYearMonthSt, 5, '0'); // 繳息年月起 5碼 *不確定*
			strField += makeFile.fillStringL(String.valueOf(iYYYMM), 5, '0'); // 繳息年月迄 5碼 *不確定*
