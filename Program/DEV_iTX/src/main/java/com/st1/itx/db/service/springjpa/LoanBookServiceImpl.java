...
@Repository
public class LoanBookServiceImpl implements LoanBookService, InitializingBean {
	private static final Logger logger = LoggerFactory.getLogger(LoanBookServiceImpl.class);
	...
	public LoanBook findById(LoanBookId loanBookId, TitaVo... titaVo) {
		...
		logger.info("findById " + dbName + " " + loanBookId);
	...
	public Slice<LoanBook> findAll(int index, int limit, TitaVo... titaVo) {
		...
		if (titaVo.length != 0)
			dbName = titaVo[0].getDataBase() != null ? titaVo[0].getDataBase() : ContentName.onLine;
		Pageable pageable = PageRequest.of(index, limit, Sort.by(Sort.Direction.ASC, "CustNo", "FacmNo", "BormNo", "BookDate"));
	...
	public Slice<LoanBook> bookDateRange(int bookDate_0, int bookDate_1, int index, int limit, TitaVo... titaVo) {
		...
		if (limit == Integer.MAX_VALUE)
			pageable = Pageable.unpaged();
		logger.info("bookDateRange " + dbName + " : " + "bookDate_0 : " + bookDate_0 + " bookDate_1 : " + bookDate_1);
		if (dbName.equals(ContentName.onDay))
	...
	public LoanBook bookBormNoFirst(int custNo_0, int facmNo_1, int bormNo_2, TitaVo... titaVo) {
		...
		if (titaVo.length != 0)
			dbName = titaVo[0].getDataBase() != null ? titaVo[0].getDataBase() : ContentName.onLine;
		logger.info("bookBormNoFirst " + dbName + " : " + "custNo_0 : " + custNo_0 + " facmNo_1 : " + facmNo_1 + " bormNo_2 : " + bormNo_2);
	...
	public Slice<LoanBook> bookCustNoRange(int custNo_0, int custNo_1, int facmNo_2, int facmNo_3, int bormNo_4, int bormNo_5, int index, int limit, TitaVo... titaVo) {
		...
		if (limit == Integer.MAX_VALUE)
			pageable = Pageable.unpaged();
		logger.info("bookCustNoRange " + dbName + " : " + "custNo_0 : " + custNo_0 + " custNo_1 : " + custNo_1 + " facmNo_2 : " + facmNo_2 + " facmNo_3 : " + facmNo_3 + " bormNo_4 : " + bormNo_4
				+ " bormNo_5 : " + bormNo_5);
		if (dbName.equals(ContentName.onDay))
			slice = loanBookReposDay
					.findAllByCustNoGreaterThanEqualAndCustNoLessThanEqualAndFacmNoGreaterThanEqualAndFacmNoLessThanEqualAndBormNoGreaterThanEqualAndBormNoLessThanEqualOrderByCustNoAscFacmNoAscBormNoAscBookDateAsc(
							custNo_0, custNo_1, facmNo_2, facmNo_3, bormNo_4, bormNo_5, pageable);
		else if (dbName.equals(ContentName.onMon))
	...
	public LoanBook holdById(LoanBookId loanBookId, TitaVo... titaVo) {
		...
		if (titaVo.length != 0)
			dbName = titaVo[0].getDataBase() != null ? titaVo[0].getDataBase() : ContentName.onLine;
		logger.info("Hold " + dbName + " " + loanBookId);
		Optional<LoanBook> loanBook = null;
		if (dbName.equals(ContentName.onDay))
	...
	public LoanBook holdById(LoanBook loanBook, TitaVo... titaVo) {
		...
		if (titaVo.length != 0)
			dbName = titaVo[0].getDataBase() != null ? titaVo[0].getDataBase() : ContentName.onLine;
		logger.info("Hold " + dbName + " " + loanBook.getLoanBookId());
		Optional<LoanBook> loanBookT = null;
		if (dbName.equals(ContentName.onDay))
	...
	public LoanBook insert(LoanBook loanBook, TitaVo... titaVo) throws DBException {
		...
		logger.info("Insert..." + dbName + " " + loanBook.getLoanBookId());
	...
	public LoanBook update(LoanBook loanBook, TitaVo... titaVo) throws DBException {
		...
		logger.info("Update..." + dbName + " " + loanBook.getLoanBookId());
		if (!empNot.isEmpty())
	...
	public void delete(LoanBook loanBook, TitaVo... titaVo) throws DBException {
		...
		if (titaVo.length != 0)
			dbName = titaVo[0].getDataBase() != null ? titaVo[0].getDataBase() : ContentName.onLine;
		logger.info("Delete..." + dbName + " " + loanBook.getLoanBookId());
		if (dbName.equals(ContentName.onDay)) {
