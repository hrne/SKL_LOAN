...
public class LM079Report extends MakeReport {
	// private static final Logger logger = LoggerFactory.getLogger(LM079Report.class);
	...
	public boolean exec(TitaVo titaVo) throws LogicException {
		...
		try {
			lLM079 = lM079ServiceImpl.findAll(titaVo, Integer.toString(iAcDate).substring(0,6));
		} catch (Exception e) {
	...
	private void exportExcel(TitaVo titaVo, List<Map<String, String>> lList, int date) throws LogicException {
		...
		makeExcel.open(titaVo, titaVo.getEntDyI(), titaVo.getKinbr(), "LM079", "B045金融機構承作「工業區閒置土地抵押貸款」統計表",
				"LM079_B045金融機構承作「工業區閒置土地抵押貸款」統計表" + showRocDate(entdy, 0).substring(0, 7), "LM079_底稿_B045金融機構承作「工業區閒置土地抵押貸款」統計表.xlsx", 1, "FOA");
