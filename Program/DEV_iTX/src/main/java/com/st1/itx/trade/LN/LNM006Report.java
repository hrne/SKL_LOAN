...
import com.st1.itx.db.service.springjpa.cm.LNM006ServiceImpl;
...
@Component("lNM006Report")
@Scope("prototype")

public class LNM006Report extends MakeReport {
	// private static final Logger logger = LoggerFactory.getLogger(LNM006Report.class);
	...
	@Autowired
	public LNM006ServiceImpl lNM006ServiceImpl;
	...
	public void exec(TitaVo titaVo) throws LogicException {
		...
		try {
			...
			this.info("LNM006ServiceImpl.findAll error = " + errors.toString());
		}
	}

	private void genExcel(TitaVo titaVo) throws LogicException {
		this.info("=========== LNM006 genExcel: ");
		this.info("LNM006 genExcel TitaVo=" + titaVo);
		...
		// 清單6：放款與應收帳款-協商戶用
		inf = "戶號;借款人ID/統編;協議編號;協議前後;額度編號;撥款序號";
		...
		try {
			...
			LNList = lNM006ServiceImpl.findAll(titaVo);
...
//			this.info("-----------------" + LNList);
			...
			makeExcel.open(titaVo, titaVo.getEntDyI(), titaVo.getKinbr(), "LNM006", "清單6：放款與應收帳款-協商戶用", "清單6：放款與應收帳款-協商戶用");
			...
			StringWriter errors = new StringWriter();
			e.printStackTrace(new PrintWriter(errors));
			this.info("LNM006ServiceImpl.genExcel error = " + errors.toString());
		}
	}
}
