 ...
 * @version 1.0.0
 */
public interface LoanChequeRepositoryHist extends JpaRepository<LoanCheque, LoanChequeId> {

	// ChequeDate >= ,AND ChequeDate <= ,AND ChequeAcct >= ,AND ChequeAcct <= ,AND
	// ChequeNo >= ,AND ChequeNo <=
	...
	public Slice<LoanCheque> findAllByCustNoIsAndStatusCodeInAndChequeDateGreaterThanEqualAndChequeDateLessThanEqualOrderByChequeAcctAscChequeNoAsc(int custNo_0, List<String> statusCode_1,
			int chequeDate_2, int chequeDate_3, Pageable pageable);
	...
	public Slice<LoanCheque> findAllByStatusCodeInAndChequeDateGreaterThanEqualAndChequeDateLessThanEqualOrderByChequeDateAscChequeAcctAscChequeNoAsc(List<String> statusCode_0, int chequeDate_1,
			int chequeDate_2, Pageable pageable);

	// ChequeDate >= ,AND ChequeDate <= ,AND ChequeAcct >= ,AND ChequeAcct <= ,AND
	// ChequeNo >= ,AND ChequeNo <= ,AND StatusCode =
	...
	// CustNo >= ,AND CustNo <= ,AND StatusCode ^i ,AND ChequeNo >= ,AND ChequeNo <=
	// ,AND ChequeDate >= ,AND ChequeDate <=
