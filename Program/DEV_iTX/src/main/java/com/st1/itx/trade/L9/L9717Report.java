...
public class L9717Report extends MakeReport {
	// private static final Logger logger = LoggerFactory.getLogger(L9717Report.class);
	...
	// length: 164
	// usually we do print() with x at 1 instead of 0 so the last symbol output is actually at 165;
	// hence the extra space to avoid print()s using newBorder.length() to pin the output on right side and gets wrong position
	...
	private void fillData(TitaVo titaVo)
	{
		...
		if (lL9717 != null && lL9717.size() != 0) {
			...
			for (Map<String, String> tLDVo : lL9717)
			{
				...
				{
					case Year:
						print(0, 2, Integer.parseInt(tLDVo.get("F3")) - 1911 + " 年撥款件");
						
						for (int i = 0; i < 8; i++)
						{
							
							int itemCnt = Integer.parseInt(tLDVo.get("F" + (4+i*2)));
							int itemAmt = Integer.parseInt(tLDVo.get("F" + (5+i*2)));
							
							print(0, 25 + i * 18, nfNum.format(itemCnt), "L");
							print(0, 39 + i * 18, nfNum.format(itemAmt), "R");			
						...
						break;
					case Agent:
						print(0, 5, tLDVo.get("F1"),"C");
						print(0, 15, tLDVo.get("F2"),"C");
			...
			switch (currentSort)
			{
				case Agent:
					...
					for (int i = 0; i < 8; i++)
					{
						...
						print(0, 25 + i * 18, nfNum.format(cnt[i]), "L");
						print(0, 39 + i * 18, nfNum.format(amt[i]), "R");			
						
					}
					
					print(1, 1, newBorder);
					print(1, newBorder.length(), "總計：" + nfNum.format(cnt[7]) + " 筆　" + nfNum.format(amt[7]), "R");
					print(1, 1, newBorder);
				...
				case LargeAmt_Agent:
					...
					break;
				case LargeAmt_Customer:
					print(1, 27, "總計： " + nfNum.format(cnt[7]) + "筆　" + nfNum.format(amt[7]), "L");
					print(1, 1, newBorder);
					break;
				default:
					break;
