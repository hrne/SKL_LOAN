...
public class L5911 extends TradeBuffer {
	// private static final Logger logger = LoggerFactory.getLogger(L5911.class);
...
/*		需求欄位
		案件編號 FacMain.CreditSysNo
		核准額度 FacMain.LineAmt，同CreditSysNo相加
		借款人戶號 FacMain.CustNo
		額度比數  同戶號+同CreditSysNo有多少不同FacmNo或ApplNo
		首次撥款日** 要找首次撥款日落在查詢範圍內的資料 FacMain.FirstDrawdownDate
		評估淨值 (猜測是同一額度相加)
		成數 (有固定算法)
		成數區間 (excel有)
		地區別名稱
		...
		地區別
		押租金
		房貸專員 原編+姓名 FacMain.BusinessOfficer
