...
public class L5801 extends TradeBuffer {
	...
	// private static final Logger logger = LoggerFactory.getLogger(L5801.class);
	...
	public ArrayList<TotaVo> run(TitaVo titaVo) throws LogicException {
		...
		if (thisMonth == 0 || lastMonth == 0) {
			webClient.sendPost(dDateUtil.getNowStringBc(), "1800", titaVo.getParam("TLRNO"), "Y", titaVo.getTxcd(),
					titaVo.getParam("TLRNO"), "L5801補貼息作業輸入參數[年月]有誤", titaVo);
		...
		webClient.sendPost(dDateUtil.getNowStringBc(), "1800", titaVo.getParam("TLRNO"), "Y", "LC009",
				titaVo.getParam("TLRNO"), "L5801補貼息申貸名冊工作檔已完成", titaVo);
		
		l5801report2.exec(thisMonth, lastMonth, titaVo);
		webClient.sendPost(dDateUtil.getNowStringBc(), "1800", titaVo.getParam("TLRNO"), "Y", "LC009",
				titaVo.getParam("TLRNO"), "L5801補貼息結清名冊終止名冊工作檔已完成", titaVo);
		
		l5801report3.exec(thisMonth, lastMonth, titaVo);
		webClient.sendPost(dDateUtil.getNowStringBc(), "1800", titaVo.getParam("TLRNO"), "Y", "LC009",
				titaVo.getParam("TLRNO"), "L5801補貼息核撥清單工作檔已完成", titaVo);
		
		l5801report4.exec(thisMonth, lastMonth, titaVo);
		webClient.sendPost(dDateUtil.getNowStringBc(), "1800", titaVo.getParam("TLRNO"), "Y", "LC009",
				titaVo.getParam("TLRNO"), "L5801補貼息核撥清單明細檔已完成", titaVo);
		this.addList(this.totaVo);
		return this.sendList();
	}
