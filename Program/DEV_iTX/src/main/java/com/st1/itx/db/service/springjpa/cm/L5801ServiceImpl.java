...
public class L5801ServiceImpl extends ASpringJpaParm implements InitializingBean {
	private static final Logger logger = LoggerFactory.getLogger(L5801ServiceImpl.class);
	...
	public List<Map<String, String>> findAll(int thisMonth, int lastMonth, TitaVo titaVo) throws Exception {
		logger.info("L5801ServiceImpl.findAll ");

		if (thisMonth == 0) {
			logger.error("L5801ServiceImpl.findAll thisMonth = 0");
			return null;
		}

		if (lastMonth == 0) {
			logger.error("L5801ServiceImpl.findAll lastMonth = 0");
			return null;
		}
