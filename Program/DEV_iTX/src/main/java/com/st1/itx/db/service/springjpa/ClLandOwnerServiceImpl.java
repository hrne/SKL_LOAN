...
public class ClLandOwnerServiceImpl extends ASpringJpaParm implements ClLandOwnerService, InitializingBean {
	...
	public Slice<ClLandOwner> findAll(int index, int limit, TitaVo... titaVo) {
		...
		if (limit == Integer.MAX_VALUE)
			...
			pageable = PageRequest.of(index, limit, Sort.by(Sort.Direction.ASC, "ClCode1", "ClCode2", "ClNo", "LandSeq", "OwnerId"));
	...
	public Slice<ClLandOwner> LandSeqEq(int clCode1_0, int clCode2_1, int clNo_2, int landSeq_3, int index, int limit, TitaVo... titaVo) {
		...
		if (dbName.equals(ContentName.onDay))
			slice = clLandOwnerReposDay.findAllByClCode1IsAndClCode2IsAndClNoIsAndLandSeqIs(clCode1_0, clCode2_1, clNo_2, landSeq_3, pageable);
		else if (dbName.equals(ContentName.onMon))
			slice = clLandOwnerReposMon.findAllByClCode1IsAndClCode2IsAndClNoIsAndLandSeqIs(clCode1_0, clCode2_1, clNo_2, landSeq_3, pageable);
		else if (dbName.equals(ContentName.onHist))
			slice = clLandOwnerReposHist.findAllByClCode1IsAndClCode2IsAndClNoIsAndLandSeqIs(clCode1_0, clCode2_1, clNo_2, landSeq_3, pageable);
		else
			slice = clLandOwnerRepos.findAllByClCode1IsAndClCode2IsAndClNoIsAndLandSeqIs(clCode1_0, clCode2_1, clNo_2, landSeq_3, pageable);
	...
	@Override
	public Slice<ClLandOwner> ownerIdEq(String ownerId_0, int index, int limit, TitaVo... titaVo) {
		String dbName = "";
		Slice<ClLandOwner> slice = null;
		if (titaVo.length != 0)
			dbName = titaVo[0].getDataBase() != null ? titaVo[0].getDataBase() : ContentName.onLine;
		...
		if (limit == Integer.MAX_VALUE)
			...
			pageable = PageRequest.of(index, limit);
		this.info("ownerIdEq " + dbName + " : " + "ownerId_0 : " + ownerId_0);
		if (dbName.equals(ContentName.onDay))
			slice = clLandOwnerReposDay.findAllByOwnerIdIs(ownerId_0, pageable);
		else if (dbName.equals(ContentName.onMon))
			slice = clLandOwnerReposMon.findAllByOwnerIdIs(ownerId_0, pageable);
		else if (dbName.equals(ContentName.onHist))
			slice = clLandOwnerReposHist.findAllByOwnerIdIs(ownerId_0, pageable);
		else
			slice = clLandOwnerRepos.findAllByOwnerIdIs(ownerId_0, pageable);
