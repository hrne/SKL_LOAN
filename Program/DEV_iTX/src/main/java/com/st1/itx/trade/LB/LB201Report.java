...
public class LB201Report extends MakeReport {
	...
	public void exec(TitaVo titaVo) throws LogicException {
		...
		try {
			...
			List<Map<String, String>> LBList = lB201ServiceImpl.findAll(titaVo);
//			this.info("-----------------" + LBList);
			if (LBList == null) {
				listCount = 0;
			} else {
	...
	private void genFile(TitaVo titaVo, List<Map<String, String>> LBList) throws LogicException {
		...
		try {
			...
			// 首筆
			strContent = "JCIC-DAT-B201-V01-458" + StringUtils.repeat(" ", 5) + strToday + "01" + StringUtils.repeat(" ", 10) + makeFile.fillStringR("審查聯絡人－" + L8ConstantEum.contact, 20, ' ')
					+ makeFile.fillStringR(L8ConstantEum.phoneNum, 16, ' ') + makeFile.fillStringR("", 20, ' ') + makeFile.fillStringR("", 16, ' ') + StringUtils.repeat(" ", 80)
					+ StringUtils.repeat(" ", 315);
				...
				for (Map<String, String> tLBVo : LBList) {
					...
					for (int j = 1; j <= 91; j++) {
						...
						case 80:
							...
							break;
						case 81:
							strField = makeFile.fillStringR(strField, 15, ' ');
							break;
...
//					}
			...
			strContent = "TRLR" + StringUtils.repeat(" ", 3) + makeFile.fillStringL(String.valueOf(sumDrawdownAmt), 13, '0') // 訂約金額(台幣)
					...
					+ StringUtils.repeat(" ", 3) + makeFile.fillStringL(String.valueOf(listCount), 9, '0') + StringUtils.repeat(" ", 456);
			makeFile.put(strContent);
	...
	private void genExcel(TitaVo titaVo, List<Map<String, String>> LBList) throws LogicException {
		...
		// B201 聯徵授信餘額月報檔
		inf = "總行代號(1~3),分行代號(4~7),交易代碼(8),帳號屬性註記(9),本筆撥款帳號(10~59)," + "金額合計(60~69),授信戶IDN/BAN(70~79),上欄IDN或BAN錯誤註記(80),負責人IDN/負責之事業體BAN(81~90),上欄IDN或BAN錯誤註記(91),"
				+ "外僑兼具中華民國國籍IDN(92~101),授信戶行業別(102~107),空白(108~110),科目別(111),科目別註記(112)," + "轉催收款(或呆帳)前原科目別(113),個人消費性貸款註記(114),融資分類(115),政府專業補助貸款分類(116~117),不計入授信項目(118),"
				+ "用途別(119),本筆撥款利率(120~127),本筆撥款開始年月(128~132),本筆撥款約定清償年月(133~137),授信餘額幣別(138~140)," + "訂約金額(台幣)(141~150),訂約金額(外幣)(151~160),循環信用註記(161),額度可否撤銷(162),上階共用額度控制編碼(163~212),"
				...
				+ "債權處理案號(321~334),債權轉讓年月/債權轉讓後原債權機構買回年月(335~339),空白(340~345),擔保品組合型態(346),擔保品(合計)鑑估值(347~356)," + "擔保品類別(357~358),國內或國際聯貸(359),聯貸合約訂定日期(360~367),聯貸參貸比例(368~371),空白(372~373),"
				...
				+ "共同債務人或債務關係人身份代號5(439),共同債務人或債務關係人身份統一編號5(440~449),上欄IDN或BAN錯誤註記(450),與主債務人關係5(451~452),就學貸款逾期案件信保基金償金額(453~457),"
				+ "空白(458~472),呆帳轉銷年月(473~477),聯貸主辦(管理)行註記(478~482),破產宣告日(或法院裁定開始清算日)(483~489),建築貸款註記(490),"
				+ "授信餘額列報1（千元）之原始金額（元）(491~494),補充揭露案件註記－案件屬性(495),補充揭露案件註記－案件情形(496~497),空白(498~506),資料所屬年月(507~511)," + "資料結束註記(512)";
