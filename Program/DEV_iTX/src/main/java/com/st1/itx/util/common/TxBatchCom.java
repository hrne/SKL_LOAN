...
//  TempVo
	...
	public ArrayList<TotaVo> run(TitaVo titaVo) throws LogicException {
		...
		if ("L3".equals(titaVo.getTxCode().substring(0, 2)) && titaVo.get("BATCHNO") != null && "BATX".equals(titaVo.get("BATCHNO").substring(0, 4)) && titaVo.getParam("RpDetailSeq1") != null) {
			// 隔日訂正，回沖至暫收可抵繳，不處理
...
// this.procStsCode 處理狀態 0.未檢核 1.不處理 2.人工處理 3.檢核錯誤 4.檢核正常 5.人工入帳 6.批次入帳 7.虛擬轉暫收
...
//				L3200 回收登錄     1.期款 2.部分償還 
		...
		if (this.tTempVo.get("EraseCnt") != null)
			txTitaVo.putParam("TXTNO", tDetail.getBatchNo().substring(4, 6) + this.tTempVo.get("EraseCnt") + parse.IntegerToString(tDetail.getDetailSeq(), 6 - this.tTempVo.get("EraseCnt").length()));
		else
			txTitaVo.putParam("TXTNO", tDetail.getBatchNo().substring(4, 6) + parse.IntegerToString(tDetail.getDetailSeq(), 6));
		...
		// 短繳金額
		String shortAmt = this.tTempVo.get("ShortAmt");
		// 溢繳金額
		String overAmt = this.tTempVo.get("OverAmt");
		// 短溢收
		if (shortAmt != null) {
	...
	private TitaVo setL3210Tita(TitaVo l3210TitaVo, BatxDetail tBatxDetail) throws LogicException {
		int iReasonCode = 0;
//		ReasonCode	     RepayType	   ProcCode             RepayCode 
//
//		00債協暫收款	     11.債協匯入款 
//		01溢繳		     X	
//		02不足利息		 01.期款		   00004-不足利息
//		03期票		     X	
//		04本金異動	     X	
//		05積欠期款		 01.期款		   00005-積欠期款
//		06即期票現金          X	
...
//      AmlRsp1 0.非可疑名單/已完成名單確認 1.需審查/確認 2.為凍結名單/未確定名單		
		if ("2".equals(this.tTempVo.get("AmlRsp1")) || "2".equals(this.tTempVo.get("AmlRsp2")))
			...
			iReasonCode = 7;
		else if ("00004".equals(tBatxDetail.getProcCode()))
			iReasonCode = 2;
		else if ("00005".equals(tBatxDetail.getProcCode()))
			iReasonCode = 5;
		else
			iReasonCode = 9;
	...
	private TitaVo setL3200Tita(TitaVo l3200TitaVo, BatxDetail tBatxDetail) throws LogicException {
		...
		l3200TitaVo.putParam("PayIntDate", this.payIntDate); // 應繳日
		...
		if (tBatxDetail.getRepayType() == 2) {
			...
			l3200TitaVo.putParam("IncludeIntFlag", "Y"); // 是否內含利息
			l3200TitaVo.putParam("UnpaidIntFlag", "N"); // 利息是否可欠繳
		...
		l3200TitaVo.putParam("TotalRepayAmt", tBatxDetail.getAcquiredAmt()); // 應收付總金額 --> 應還金額
	...
	private TitaVo setL3420Tita(TitaVo l3420TitaVo, BatxDetail tBatxDetail) throws LogicException {
		...
		l3420TitaVo.putParam("TotalRepayAmt", tBatxDetail.getAcquiredAmt()); // 應收付總金額 --> 應還金額
...
//		  DisacctAmt    入暫收金額  DECIMAL(14)         OverRpAmt
		...
		BatxDetail tDetail = batxDetailService.holdById(tBatxDetailId);
		...
		if (this.txBuffer.getTxCom().getTxRsut() == 0) { // 交易成功
			if (titaVo.isHcodeNormal()) { // 正常交易
				...
				if (titaVo.isTrmtypBatch()) { // 批次入帳
					// 虛擬轉暫收功能、入暫收金額=0
					if ("2".equals(titaVo.get("FunctionCode")) || disacctAmt.compareTo(BigDecimal.ZERO) == 0) {
						tDetail.setProcStsCode("6"); // 6.批次入帳
						unfinishCnt = -1;
					} else {
						tDetail.setProcStsCode("7"); // 7.虛擬轉暫收
						unfinishCnt = 0;
					}
				} else {
			...
			// 交易失敗
			this.tTempVo.putParam("ErrorMsg", this.txBuffer.getTxCom().getMsgId() + this.txBuffer.getTxCom().getErrorMsg()); // 錯誤訊息
...
//		BatxDetail's MediaDate MediaCode MediaSeq to find ACH/POST (僅一個媒體Table需產出多媒體File者才需區分MediaCode)
		...
		if (lBankDeductDtl != null && lBankDeductDtl.size() != 0) {
			for (BankDeductDtl tBankDeductDtl : lBankDeductDtl) {
				tBankDeductDtl = bankDeductDtlService.holdById(tBankDeductDtl);
...
//		BatxDetail's MediaDate MediaCode MediaSeq to find 15/un15 
		...
		sEmpDeductDtl = empDeductDtlService.mediaSeqEq(tBatxDetail.getMediaDate() + 19110000, tBatxDetail.getMediaKind(), tBatxDetail.getMediaSeq(), this.index, Integer.MAX_VALUE, titaVo);
		...
		if (lEmpDeductDtl != null && lEmpDeductDtl.size() != 0) {
			for (EmpDeductDtl tEmpDeductDtl : lEmpDeductDtl) {
				tEmpDeductDtl = empDeductDtlService.holdById(tEmpDeductDtl);
				tEmpDeductDtl.setAcdate(acDate);
				try {
					empDeductDtlService.update(tEmpDeductDtl);
				} catch (DBException e) {
...
// this.repayType 還款類別 1.期款 2.部分償還 3.結案 4.帳管費 5.火險費 6.契變手續費 7.法務費 9.其他 11.債協匯入款(虛擬帳號為9510500NNNNNNN)
		...
		case 1:
			...
			if (this.shortAmt.compareTo(BigDecimal.ZERO) > 0) {
				if (this.shortAmt.compareTo(this.principal) > 0) {
					this.checkMsg = "不足期金: " + this.shortAmt;
					this.procCode = "00005";
				} else {
		...
		case 3:
			...
			if (this.overAmt.add(this.tavAmt).subtract(this.tmpAmt).compareTo(BigDecimal.ZERO) > 0) {
				this.checkMsg = "結案有溢繳款: " + this.overAmt.add(this.tavAmt).subtract(this.tmpAmt);
				apendcheckMsgAmounts(tBatxDetail, titaVo);
				this.procStsCode = "2"; // 2.人工處理
				break;
			}
		...
		case 7:
			...
			// 處理說明: ：金額不足: 999,999,999,999
			if (this.repayTypeFee.compareTo(BigDecimal.ZERO) == 0) {
				this.checkMsg = "金額不足: " + this.unPayRepayTypeFee.subtract(tBatxDetail.getRepayAmt().subtract(this.tavAmt));
				break;
			}
			break;
		// 09-其他
		...
		this.info("償還本利 unPayLoan           = " + this.unPayLoan);
		...
		this.info("結案記號 closeFg             = " + this.closeFg);
		this.info("檢核訊息  checkMsg           = " + this.checkMsg);
		this.info("錯誤訊息  errorMsg           = " + this.errorMsg);
		this.info("處理狀態  procStsCode        = " + this.procStsCode);
		this.info("處理代碼  ProcCod            = " + this.procCode);
	...
	private void apendcheckMsgAmounts(BatxDetail tBatxDetail, TitaVo titaVo) throws LogicException {
		if (this.unPayLoan.compareTo(BigDecimal.ZERO) > 0)
			this.checkMsg = this.checkMsg + ", 償還本利:" + this.unPayLoan;
		//
	...
	/* 匯款轉帳設定還款類別為費用類別 */
	private void bankRmtRepayTypeFee(BatxDetail tBatxDetail, TitaVo titaVo) throws LogicException {
//		還款類別
...
// 債協匯入款:95105     ==> 11-債協匯入款
	...
	private void loanBookRepayType(BatxDetail tDetail, TitaVo titaVo) throws LogicException {
		this.info("CASE 3-1!!!!");
		Slice<LoanBook> loanBookList = loanBookService.bookDateRange(tDetail.getEntryDate(), tDetail.getEntryDate(), this.index, Integer.MAX_VALUE);
	...
	private void facCloseRepayType(BatxDetail tDetail, TitaVo titaVo) throws LogicException {
		this.info("CASE 3-2!!!!");
		Slice<FacClose> facCloseList = facCloseService.findCustNo(tDetail.getCustNo(), this.index, Integer.MAX_VALUE); // 清償作業檔
		// 還清金額需相同
		if (facCloseList != null) {
			for (FacClose tFacClose : facCloseList.getContent()) {
				if (tFacClose.getCloseAmt().equals(tDetail.getRepayAmt())) {
	...
	private void initialProcNote(TempVo t, TitaVo titaVo) throws LogicException {
		...
		if (t.get("PayIntDate") != null) {
			...
			this.payIntDate = parse.stringToInteger(t.get("PayIntDate"));
		}
...
// this.feeRepayType 費用還款類別
		...
		// call 應繳試算
		try {
			baTxList = baTxCom.settleUnPaid(tDetail.getEntryDate(), this.payIntDate, tDetail.getCustNo(), this.repayFacmNo, 0, tDetail.getRepayCode(), this.repayType, tDetail.getRepayAmt(), titaVo);
		...
		if (this.repayType == 3) {
			if ("Y".equals(this.collectFlag)) {
				oListCloseBreach = loanCloseBreachCom.getCloseBreachAmtAll(tDetail.getCustNo(), this.repayFacmNo, 0, iListCloseBreach, titaVo);
			} else {
				oListCloseBreach = loanCloseBreachCom.getCloseBreachAmtPaid(tDetail.getCustNo(), this.repayFacmNo, 0, iListCloseBreach, titaVo);
			}
			// 輸出清償違約金
