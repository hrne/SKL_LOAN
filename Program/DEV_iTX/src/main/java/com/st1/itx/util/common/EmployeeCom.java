...
//        等於 21    &&     第一碼等於 F,G,J,Z
//        等於 31    &&     第一碼等於   K,Z
//                          不等於 21,31,1C   &&   不等於   TU0036,TU0097
		String agLevel = " ";
		if (tCdEmp.getAgLevel() != null && tCdEmp.getAgLevel().length() > 0) {
			agLevel = tCdEmp.getAgLevel().substring(0, 1);
		}

		if ("21".equals(tCdEmp.getCommLineCode())) {
			if ("F".equals(agLevel) || "G".equals(agLevel) || "J".equals(agLevel) || "Z".equals(agLevel)) {
				isSalary = true;
			}
		}

		if ("31".equals(tCdEmp.getCommLineCode())) {
			if ("K".equals(agLevel) || "Z".equals(agLevel)) {
				isSalary = true;
			}
		}

		agLevel = tCdEmp.getAgLevel();

		if (!("21".equals(agLevel) || "31".equals(agLevel) || "1C".equals(agLevel)) && (!("TU0036".equals(tCdEmp.getAgPostIn()) || "TU0097".equals(tCdEmp.getAgPostIn())))) {
			isSalary = true;
		}
