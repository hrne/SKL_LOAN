 ...
 * @version 1.0.0
 */
public interface NegMainRepositoryHist extends JpaRepository<NegMain, NegMainId> {

	// CaseKindCode= , AND CustLoanKind= , AND Status= , AND CustNo=
	public Slice<NegMain> findAllByCaseKindCodeIsAndCustLoanKindIsAndStatusIsAndCustNoIsOrderByCustNoDescCaseSeqAsc(String caseKindCode_0, String custLoanKind_1, String status_2, int custNo_3,
			Pageable pageable);
	...
	// Status^i , AND IsMainFin= , AND NextPayDate>= , AND NextPayDate<= , AND
	// CustNo=
	public Slice<NegMain> findAllByStatusInAndIsMainFinIsAndNextPayDateGreaterThanEqualAndNextPayDateLessThanEqualAndCustNoIsOrderByCustNoDescCaseSeqAsc(List<String> status_0, String isMainFin_1,
			int nextPayDate_2, int nextPayDate_3, int custNo_4, Pageable pageable);

	// Status^i , AND IsMainFin= , AND NextPayDate>= , AND NextPayDate<=
	public Slice<NegMain> findAllByStatusInAndIsMainFinIsAndNextPayDateGreaterThanEqualAndNextPayDateLessThanEqualOrderByCustNoDescCaseSeqAsc(List<String> status_0, String isMainFin_1,
			int nextPayDate_2, int nextPayDate_3, Pageable pageable);
