 ...
 * @version 1.0.0
 */
public interface AcLoanRenewRepositoryHist extends JpaRepository<AcLoanRenew, AcLoanRenewId> {

  // CustNo = ,AND NewFacmNo >= ,AND NewFacmNo <= ,AND NewBormNo >= ,AND NewBormNo <=
  public Slice<AcLoanRenew> findAllByCustNoIsAndNewFacmNoGreaterThanEqualAndNewFacmNoLessThanEqualAndNewBormNoGreaterThanEqualAndNewBormNoLessThanEqualOrderByNewFacmNoAscNewBormNoAsc(int custNo_0, int newFacmNo_1, int newFacmNo_2, int newBormNo_3, int newBormNo_4, Pageable pageable);

  // CustNo >= ,AND CustNo <= ,AND OldFacmNo >= ,AND OldFacmNo <= ,AND NewFacmNo >= ,AND NewFacmNo <= , AND AcDate >=, AND AcDate <=
  public Slice<AcLoanRenew> (int custNo_0, int custNo_1, int oldFacmNo_2, int oldFacmNo_3, int newFacmNo_4, int newFacmNo_5, int acDate_6, int acDate_7, Pageable pageable);
