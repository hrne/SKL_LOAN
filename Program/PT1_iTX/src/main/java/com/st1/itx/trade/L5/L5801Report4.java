...
//				"L5801_底稿_催收款明細表.xlsx", "工作表1", this.showRocDate(thisMonth * 100 + 1, 5));
		...
		makeExcel.open(titaVo, titaVo.getEntDyI(), titaVo.getKinbr(), "L5801", "補貼息核撥清單明細檔"+ ithisMonth, "L5801補貼息核撥清單明細檔" + ithisMonth, "補貼息核撥清單明細檔" + ithisMonth);
...
//			makeExcel.setValue(printRow, 1, "本月無資料", fontStyleVo);
			...
			for (Map<String, String> mapL5801 : listL5801) {
				...
				String Kind = "";
				String Rate = mapL5801.get("F4");//補貼利率
				switch (ProjectKind) {
				case "1":
