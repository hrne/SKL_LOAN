...
@Service("L4605Batch")
@Scope("prototype")
public class L4605Batch extends TradeBuffer {
	/* 日期工具 */
	@Autowired
...
//				file檔為NewInusNo
				...
				tmpInsu tmp = new tmpInsu(parse.stringToInteger(tempOccursList.get("CustNo")),
						...
						tempOccursList.get("InsuCustId"));
//				Initial
				error.put(tmp, 0);

				if (iInsuEndMonth != parse.stringToInteger(tempOccursList.get("FireInsuMonth"))) {
					error.put(tmp, 1);
					this.info("iInsuEndMonth=" + iInsuEndMonth + ",FireInsuMonth="
							+ parse.stringToInteger(tempOccursList.get("FireInsuMonth")));
					continue;
				}
