...
public class CdAcCode implements Serializable {
	...
	// 業務科目記號
	/*
	 * 0:非業務科目（可經由[其他傳票輸入]交易入帳）1:資負明細科目（放款、催收款項...等，不可經由[其他傳票輸入]交易入帳)※資負明細科目1:列入[
	 * 資負明細月報]項目2:可由[未銷帳餘額明細]查詢未銷明細3:列入[資負明細每日餘額檔(含所屬會科、帳冊別及利率)](資負明細業務科目)4:列入[
	 * 會計餘額檢核表(會計檔餘額、銷帳檔餘額、業務檔餘額)]項目
	 */
	...
	/**
	 * 業務科目記號<br>
	 * 0:非業務科目（可經由[其他傳票輸入]交易入帳） 1:資負明細科目（放款、催收款項...等，不可經由[其他傳票輸入]交易入帳) ※資負明細科目
	 * 1:列入[資負明細月報]項目 2:可由[未銷帳餘額明細]查詢未銷明細 3:列入[資負明細每日餘額檔(含所屬會科、帳冊別及利率)](資負明細業務科目)
	...
	/**
	 * 銷帳科目記號<br>
	 * 0:非銷帳科目 1:會計銷帳科目(銷帳編號由系統自編,ex:應收利息) 2:業務銷帳科目(銷帳編號由業務自編,ex:暫付及待結轉帳項－火險保費)
	 * 
	 * @return Integer
	 */
	...
	/**
	 * 日結餘額檢查記號<br>
	 * 0:不檢查 1:不過餘額(借貸分由放款及核心系統出帳) 2:應&amp;gt;=0 3:應=0 4:應&amp;lt;=0
	 * 
	 * @return Integer
	 */
	...
	public String toString() {
		return "CdAcCode [cdAcCodeId=" + cdAcCodeId + ", acNoItem=" + acNoItem + ", acctCode=" + acctCode + ", acctItem=" + acctItem + ", classCode=" + classCode + ", acBookFlag=" + acBookFlag
				+ ", dbCr=" + dbCr + ", acctFlag=" + acctFlag + ", receivableFlag=" + receivableFlag + ", clsChkFlag=" + clsChkFlag + ", inuseFlag=" + inuseFlag + ", acNoCodeOld=" + acNoCodeOld
