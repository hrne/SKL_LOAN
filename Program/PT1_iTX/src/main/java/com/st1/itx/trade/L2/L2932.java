...
//			lLoanBorMain = new ArrayList<LoanBorMain>();
	...
	private void setTotaB(FacMain tFacMain, TitaVo titaVo) throws LogicException {
		...
		occursList.putParam("OOBBreachFlag", tFacMain.getBreachFlag()); // 是否綁約
		...
		if ("Y".equals(tFacMain.getBreachFlag())) {
			occursList.putParam("OOBBreachCode", tFacMain.getBreachCode()); // 違約適用方式
			if (tFacMain.getProhibitMonth() > 0 && tFacMain.getFirstDrawdownDate() > 0) {
				dDateUtil.init();
				dDateUtil.setDate_1(tFacMain.getFirstDrawdownDate());
				dDateUtil.setMons(tFacMain.getProhibitMonth());
			...
			occursList.putParam("OOBBreachStartPercent", tFacMain.getBreachStartPercent()); // 還款起算比例
			occursList.putParam("OOBBreachPercent", tFacMain.getBreachPercent()); // 違約金百分比
			occursList.putParam("OOBBreachDecreaseMonth", tFacMain.getBreachDecreaseMonth()); // 分段月數
			occursList.putParam("OOBBreachDecrease", tFacMain.getBreachDecrease()); // 遞減百分比
			occursList.putParam("OOBBreachGetCode", tFacMain.getBreachGetCode()); // 違約金收取方式
		}
