...
public class L6046 extends TradeBuffer {
	...
	public ArrayList<TotaVo> run(TitaVo titaVo) throws LogicException {
		...
		if (!iCustId.isEmpty()) {
			slCustDataCtrl = sCustDataCtrlService.applMarkCustId(Arrays.asList(2), iCustId, this.index, this.limit,
					titaVo);
			if (slCustDataCtrl == null) {
				...
				slCustDataCtrl = sCustDataCtrlService.applMarkCustUKey(Arrays.asList(2), wkCustUKey, this.index,
						this.limit, titaVo);
			}
		} else {
			if (iCustNo > 0) {
				slCustDataCtrl = sCustDataCtrlService.applMarkCustNo(Arrays.asList(2), iCustNo, this.index,
						this.limit, titaVo);
			} else {
				slCustDataCtrl = sCustDataCtrlService.applMarkAll(Arrays.asList(2), this.index, this.limit, titaVo);
			}
		}
		...
		for (CustDataCtrl tCustDataCtrl : lCustDataCtrl) {
			...
			if (tCustDataCtrl.getCreateDate() != null) {
				...
				// 宣告
				ts = tCustDataCtrl.getCreateDate();
				this.info("ts = " + ts);
			...
			if (tCustDataCtrl.getCreateEmpNo() != null) {
				TlrNo = tCustDataCtrl.getCreateEmpNo();
			...
			searchTXCDs.add("L2704");
			searchTXCDs.add("L2074");
...
//			occurslist.putParam("OOCustUKey", tCustMain.getCustUKey());
			occurslist.putParam("OOCustId", tCustDataCtrl.getCustId());
			occurslist.putParam("OOCustNo", tCustDataCtrl.getCustNo());
			occurslist.putParam("OOCustName", tCustDataCtrl.getCustName());
