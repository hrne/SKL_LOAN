...
public class LNM39GPReport extends MakeReport {
	...
	public boolean exec(TitaVo titaVo) throws LogicException {
		...
		try {
			LNM39GPList = lNM39GPServiceImpl.findAll(dataMonth, titaVo);
		} catch (Exception e) {
			StringWriter errors = new StringWriter();
			e.printStackTrace(new PrintWriter(errors));
			this.error("LNM39GPReport LNM39GPServiceImpl.findAll error = " + errors.toString());
			return false;
		}
		...
		try {
			...
			StringWriter errors = new StringWriter();
			e.printStackTrace(new PrintWriter(errors));
			this.error("LNM39GPReport.genFile error = " + errors.toString());
			return false;
		}
	...
	private void genFile(TitaVo titaVo, List<Map<String, String>> L7List) throws LogicException {
		...
		try {
			...
			// 產製[控制檔]
			makeFileC.open(titaVo, titaVo.getEntDyI(), titaVo.getKinbr(), "LNFGPIDX", "LNM39GP 欄位清單７控制檔", "LNFGP.IDX",
					1); // UTF-8
