...
//				暫無用處
				...
				String processTime = "";
				if (result.get("F22").length() == 6) {
					processTime = result.get("F22").substring(0, 2) + ":" + result.get("F22").substring(2, 4) + ":"
							+ result.get("F22").substring(4, 6);
				} else {
					processTime = result.get("F22");
				}
				occursList.putParam("OOProcessTime", processTime);
