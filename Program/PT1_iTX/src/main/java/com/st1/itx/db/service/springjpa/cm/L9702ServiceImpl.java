...
public class L9702ServiceImpl extends ASpringJpaParm implements InitializingBean {
	...
	public List<Map<String, String>> finddbf(TitaVo titaVo) throws Exception {
		...
		int acDate_st = Integer.valueOf(titaVo.get("ACCTDATE_ST")) + 19110000;
		int acDate_ed = Integer.valueOf(titaVo.get("ACCTDATE_ED")) + 19110000;
		
		String sYearMonth = String.valueOf(acDate_st / 100);
		String eYearMonth = String.valueOf(acDate_ed / 100);
		
//		String ACCTDATE_ST = String.valueOf(acDate_ed);
//		String ACCTDATE_ED = String.valueOf(acDate_ed);


		int acYearBc = acDate_ed / 10000;
		int acMonthBc = (acDate_ed / 100) % 100;
		if(acMonthBc == 12) {
			acDate_ed = (acYearBc + 1) * 10000 + 101;
		}else {
			acDate_ed = (acYearBc * 10000) + (acMonthBc + 1) * 100 +1;
		}
		this.info("sYearMonth = " + sYearMonth);
		this.info("eYearMonth = " + eYearMonth);
		this.info("acDate_st = " + acDate_st);
		this.info("acDate_ed = " + acDate_ed);

		
		this.info("l9702.finddbf");
		String sql = "	";
		sql += "	WITH \"tempMain\" AS (";
		sql += "		SELECT M.\"YearMonth\"";
		sql += "			  ,M.\"CustNo\"";
		sql += "			  ,M.\"FacmNo\"";
		sql += "			  ,M.\"BormNo\"";
		sql += "			  ,M.\"DepartmentCode\"";
		sql += "			  ,DECODE(M.\"YearMonth\", :eyymm ,M.\"LoanBalance\",0) AS \"LoanBalance\"";
		sql += "			  ,DECODE(M.\"YearMonth\", :eyymm ,L.\"DrawdownAmt\",0) AS \"DrawdownAmt\"";
		sql += "			  ,L.\"MaturityDate\"";
		sql += "			  ,DECODE(M.\"YearMonth\", :eyymm ,LR.\"FitRate\",0) AS \"FitRate\"";
		sql += "			  ,DECODE(M.\"YearMonth\", :eyymm ,M.\"StoreRate\",0) AS \"Rate\"";
		...
		sql += "				WHERE \"EffectDate\" <= :eyymmdd";
		sql += "			) WHERE \"Seq\" = 1 ";
		sql += "		) LR ON LR.\"CustNo\" = M.\"CustNo\"";
		sql += "			AND LR.\"FacmNo\" = M.\"FacmNo\"";
		sql += "			AND LR.\"BormNo\" = M.\"BormNo\"";
		sql += "		WHERE M.\"YearMonth\" BETWEEN :syymm AND :eyymm";
		sql += "	),\"tempMain2\" AS (";
		sql += "		SELECT \"CustNo\"";
		sql += "			  ,\"FacmNo\"";
		sql += "			  ,\"BormNo\"";
		...
		sql += "		  ,M2.\"MaturityDate\"";
		sql += "		  ,M.\"DrawdownAmt\"";
		sql += "		  ,DECODE(M.\"Rate\",0,R.\"Rate\",M.\"Rate\") AS \"Rate\"";
		sql += "		  ,M.\"LoanBal\"";
		sql += "		  ,T.\"Interest\"";
		sql += "		  ,M2.\"DepartmentCode\"";
		sql += "	FROM (";
		sql += "		SELECT \"CustNo\"";
		sql += "			  ,\"FacmNo\"";
		sql += "			  ,SUM(\"Interest\" + \"DelayInt\" + \"BreachAmt\") AS \"Interest\"";
		sql += "		FROM (";
		sql += "			SELECT \"CustNo\"";
		sql += "				  ,\"FacmNo\"";
		sql += "				  ,SUM(DECODE(\"TitaHCode\",3,-\"Interest\",1,-\"Interest\",\"Interest\")) AS \"Interest\"";
		sql += "				  ,SUM(DECODE(\"TitaHCode\",3,-\"DelayInt\",1,-\"DelayInt\",\"DelayInt\")) AS \"DelayInt\"";
		sql += "				  ,SUM(DECODE(\"TitaHCode\",3,-\"BreachAmt\",1,-\"BreachAmt\",\"BreachAmt\")) AS \"BreachAmt\"";
		sql += "			FROM \"LoanBorTx\"";
		sql += "			WHERE \"AcDate\" BETWEEN :syymmdd AND :eyymmdd ";
		sql += "			  AND \"FacmNo\" > 0";
		sql += "			GROUP BY \"CustNo\"";
		sql += "					,\"FacmNo\"";
		sql += "			UNION";
		...
		sql += "				  ,0 AS \"BreachAmt\"";
		sql += "			FROM \"MonthlyLoanBal\"";
		sql += "			WHERE \"YearMonth\" BETWEEN :syymm AND :eyymm";
		sql += "			  AND \"LoanBalance\" > 0";
		sql += "		)";
		sql += "		GROUP BY \"CustNo\"";
		sql += "				,\"FacmNo\"";
		...
		sql += "		SELECT \"CustNo\"";
		sql += "			  ,\"FacmNo\"";
		sql += "			  ,SUM(DECODE(\"YearMonth\", :eyymm ,\"DrawdownAmt\",0)) AS \"DrawdownAmt\"";
		sql += "			  ,MAX( ";
		sql += "				CASE ";
		sql += "				  WHEN \"YearMonth\" = :eyymm ";
		sql += "				   AND \"LoanBalance\" > 0 THEN \"FitRate\"";
		sql += "				ELSE 0 END ";
		sql += "			  ) AS \"Rate\"";
		sql += "			  ,SUM(DECODE(\"YearMonth\", :eyymm ,\"LoanBalance\",0)) AS \"LoanBal\"";
		sql += "		FROM \"tempMain\"";
		sql += "		WHERE \"YearMonth\" = :eyymm ";
		sql += "		GROUP BY \"CustNo\"";
		sql += "				,\"FacmNo\"";
		sql += "	) M ON M.\"CustNo\" = T.\"CustNo\"";
		sql += "	   AND M.\"FacmNo\" = T.\"FacmNo\"";
		sql += "	LEFT JOIN (";
		sql += "		SELECT \"CustNo\"";
		sql += "			  ,\"FacmNo\"";
		sql += "			  ,MAX(\"MaturityDate\") AS \"MaturityDate\"";
		sql += "			  ,MAX(\"DepartmentCode\") AS \"DepartmentCode\"";
		sql += "		FROM \"tempMain\"";
		sql += "		GROUP BY \"CustNo\"";
		sql += "				,\"FacmNo\"";
		sql += "	) M2 ON M2.\"CustNo\" = T.\"CustNo\"";
		sql += "		AND M2.\"FacmNo\" = T.\"FacmNo\"";
		sql += "	LEFT JOIN \"tempMain2\" R ON R.\"CustNo\" = T.\"CustNo\"";
		sql += "							 AND R.\"FacmNo\" = T.\"FacmNo\"";
		sql += "							 AND R.\"Seq\" = 1";
...
//		sql += "              ,F.\"DepartmentCode\"";
		...
		EntityManager em = this.baseEntityManager.getCurrentEntityManager(titaVo);
		query = em.createNativeQuery(sql);
		query.setParameter("syymm", sYearMonth);
		query.setParameter("eyymm", eYearMonth);
		query.setParameter("syymmdd", acDate_st);
		query.setParameter("eyymmdd", acDate_ed);
		return this.convertToMap(query);
	}
