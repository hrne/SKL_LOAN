...
public class L8417 extends TradeBuffer {
	...
	public ArrayList<TotaVo> run(TitaVo titaVo) throws LogicException {
		...
		sJcicZ054 = sJcicZ054Service.findAll(index, limit, titaVo);
		...
		if (sJcicZ054 != null) {
			for (JcicZ054 xJcicZ054 : sJcicZ054) {
				if ((iSubmitType == 1 && xJcicZ054.getOutJcicTxtDate() == 0) || (iSubmitType == 3 && xJcicZ054.getActualFilingDate() == 0)) {
					...
					iActualFilingDate = xJcicZ054.getActualFilingDate();
					if (iActualFilingDate == 0) {
						occursListB.putParam("OOActualFilingDate", "");
					} else {
						occursListB.putParam("OOActualFilingDate", iActualFilingDate);
					}
					occursListB.putParam("OOActualFilingMark", xJcicZ054.getActualFilingMark());
					this.totaVo.addOccursList(occursListB);
				} else if (iSubmitType == 2) {
					if (xJcicZ054.getOutJcicTxtDate() == iJcicDate && xJcicZ054.getActualFilingDate() == 0) {
						OccursList occursListB = new OccursList();
						occursListB.putParam("OOChainTxCd", "L8315");
						occursListB.putParam("OOHistoryTxCd", "L8045d");
						occursListB.putParam("OOCustId", xJcicZ054.getCustId());
						occursListB.putParam("OOSubmitKey", xJcicZ054.getSubmitKey());
						occursListB.putParam("OOSubmitKeyX", dealBankName(xJcicZ054.getSubmitKey(), titaVo));
						occursListB.putParam("OORcDate", xJcicZ054.getRcDate());
