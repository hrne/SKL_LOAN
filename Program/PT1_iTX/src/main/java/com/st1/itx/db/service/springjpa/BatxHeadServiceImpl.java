...
public class BatxHeadServiceImpl extends ASpringJpaParm implements BatxHeadService, InitializingBean {
	...
	public BatxHead batchNoDescFirst(int acDate_0, String batchNo_1, TitaVo... titaVo) {
		...
		if (titaVo.length != 0)
			dbName = titaVo[0].getDataBase() != null ? titaVo[0].getDataBase() : ContentName.onLine;
		this.info("batchNoDescFirst " + dbName + " : " + "acDate_0 : " + acDate_0 + " batchNo_1 : " + batchNo_1);
		Optional<BatxHead> batxHeadT = null;
		if (dbName.equals(ContentName.onDay))
			batxHeadT = batxHeadReposDay.findTopByAcDateIsAndBatchNoLikeOrderByBatchNoDesc(acDate_0, batchNo_1);
		else if (dbName.equals(ContentName.onMon))
			batxHeadT = batxHeadReposMon.findTopByAcDateIsAndBatchNoLikeOrderByBatchNoDesc(acDate_0, batchNo_1);
		else if (dbName.equals(ContentName.onHist))
			batxHeadT = batxHeadReposHist.findTopByAcDateIsAndBatchNoLikeOrderByBatchNoDesc(acDate_0, batchNo_1);
		else
			batxHeadT = batxHeadRepos.findTopByAcDateIsAndBatchNoLikeOrderByBatchNoDesc(acDate_0, batchNo_1);
	...
	public BatxHead insert(BatxHead batxHead, TitaVo... titaVo) throws DBException {
		...
		if (this.findById(batxHead.getBatxHeadId(), titaVo) != null)
