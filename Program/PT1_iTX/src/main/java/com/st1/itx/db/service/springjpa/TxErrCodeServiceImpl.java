...
public class TxErrCodeServiceImpl extends ASpringJpaParm implements TxErrCodeService, InitializingBean {
	...
	public Slice<TxErrCode> findAll(int index, int limit, TitaVo... titaVo) {
		...
		if (limit == Integer.MAX_VALUE)
			pageable = PageRequest.of(0, Integer.MAX_VALUE, Sort.by(Sort.Direction.ASC, "ErrCode"));
		else
			pageable = PageRequest.of(index, limit, Sort.by(Sort.Direction.ASC, "ErrCode"));
	...
	public Slice<TxErrCode> findbyErrCode(String errCode_0, int index, int limit, TitaVo... titaVo) {
		...
		this.info("findbyErrCode " + dbName + " : " + "errCode_0 : " + errCode_0);
		if (dbName.equals(ContentName.onDay))
			slice = txErrCodeReposDay.findAllByErrCodeLike(errCode_0, pageable);
		else if (dbName.equals(ContentName.onMon))
			slice = txErrCodeReposMon.findAllByErrCodeLike(errCode_0, pageable);
		else if (dbName.equals(ContentName.onHist))
			slice = txErrCodeReposHist.findAllByErrCodeLike(errCode_0, pageable);
		else
			slice = txErrCodeRepos.findAllByErrCodeLike(errCode_0, pageable);
	...
	public TxErrCode insert(TxErrCode txErrCode, TitaVo... titaVo) throws DBException {
		...
		if (this.findById(txErrCode.getErrCode(), titaVo) != null)
			throw new DBException(2);

		if (!empNot.isEmpty())
			txErrCode.setCreateEmpNo(empNot);

		if (txErrCode.getLastUpdateEmpNo() == null || txErrCode.getLastUpdateEmpNo().isEmpty())
