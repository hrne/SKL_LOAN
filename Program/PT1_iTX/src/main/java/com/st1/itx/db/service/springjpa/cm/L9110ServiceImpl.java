...
public class L9110ServiceImpl extends ASpringJpaParm implements InitializingBean {
	...
	public List<Map<String, String>> queryBuildingLand(TitaVo titaVo, String applNo) throws Exception {
		...
		sql += "                 ORDER BY ";
		sql += "                     S1.\"LandSeq\" ASC ";
		sql += "             ) = 1 THEN TO_CHAR(S1.\"BdArea\") ELSE ' ' END AS \"F6主建物面積\"   ";
		sql += " ,CASE WHEN ROW_NUMBER() OVER( ";
		sql += "                 PARTITION BY S1.\"ClCode1\", S1.\"ClCode2\", S1.\"ClNo\" ";
		sql += "                 ORDER BY ";
		sql += "                     S1.\"LandSeq\" ASC ";
		sql += "             ) = 1 THEN TO_CHAR(S1.\"BdSubArea\") ELSE ' ' END  AS \"F7附屬建物\"    ";
		sql += " ,CASE WHEN ROW_NUMBER() OVER( ";
		sql += "                 PARTITION BY S1.\"ClCode1\", S1.\"ClCode2\", S1.\"ClNo\" ";
		sql += "                 ORDER BY ";
		sql += "                     S1.\"LandSeq\" ASC ";
		sql += "             ) = 1 THEN TO_CHAR(S1.\"BdPublicArea\") ELSE ' ' END  AS \"F8公設面積\"  ";
		sql += " ,CASE WHEN ROW_NUMBER() OVER( ";
		sql += "                 PARTITION BY S1.\"ClCode1\", S1.\"ClCode2\", S1.\"ClNo\" ";
		sql += "                 ORDER BY ";
		sql += "                     S1.\"LandSeq\" ASC ";
		sql += "             ) = 1 THEN TO_CHAR(S1.\"BdParkingArea\") ELSE ' ' END  AS \"F9車位\"   ";
		sql += " ,CASE WHEN ROW_NUMBER() OVER( ";
		sql += "                 PARTITION BY S1.\"ClCode1\", S1.\"ClCode2\", S1.\"ClNo\" ";
		sql += "                 ORDER BY ";
		sql += "                     S1.\"LandSeq\" ASC ";
		sql += "             ) = 1 THEN TO_CHAR(S1.\"SettingAmt\")    ";
		sql += "	WHEN S1.\"BdLandItem\" = 2 THEN TO_CHAR(S1.\"SettingAmt\") ELSE ' ' END  AS \"F10設定金額\" ";
		sql += " , CASE WHEN S1.\"BdLandItem\" = 2 THEN TO_CHAR(S1.\"LandArea\") ELSE ' ' END  AS \"F11土地面積\"   ";
		sql += " ,S1.\"ClCode1\" AS \"F12ClCode1\"    ";
		sql += " ,S1.\"ClCode2\" AS \"F13ClCode2\"    ";
		sql += " ,S1.\"ClNo\" AS \"F14ClNo\"    ";
		sql += " ,S1.\"BdEvaunitprice\" AS F15鑑定單價   ";
		sql += " ,S1.\"BdSellerName\" AS F16賣方姓名   ";
		sql += " ,S1.\"BdSellerId\" AS \"F17賣方ID\"    ";
		sql += " ,S1.\"LandSeq\" AS \"F18LandSeq\"   ";
		sql += " ,S1.\"LandCityItem\" AS F19土地縣市    ";
		sql += " ,S1.\"LandAreaItem\" AS F20土地鄉鎮區   ";
		sql += " ,S1.\"LandIrItem\" AS F21土地段小段    ";
		sql += " ,S1.\"LandLandNo\" AS F22土地地號   ";
		sql += " ,S1.\"LandTransFeredYear\" AS F23土地年度   ";
		sql += " ,S1.\"LandLastTransferedAmt\" AS F24土地前次移轉   ";
		sql += " ,S1.\"LandEvaunitprice\" AS F25土地鑑定單價  ";
		sql += " ,S1.\"LandOwnerPart\" AS F26土地持份比例    ";
		sql += " ,S1.\"LandOwnerTotal\" AS F27土地持份總比例    ";
		sql += " ,S1.\"LandClItem\" AS F28土地擔保品別  ";
		sql += " ,S1.\"BdLandItem\" AS F29類別  ";
		sql += " ,S1.\"Cnt\" AS F30筆數  ";
		sql += " ,S1.\"TotalLandArea\" AS F31土地總面積  ";
		sql += " FROM ";
		sql += "     (( ";
		sql += "         SELECT ";
		sql += "             ";
...
//		sql += "         AND CF.\"MainFlag\" = 'Y' "; // -- 主要擔保品
	...
	public List<Map<String, String>> queryInsu(TitaVo titaVo, String applNo) throws Exception {
		...
		sql += "   	        WHEN FM.\"FirstDrawdownDate\" > 0 AND IR.\"InsuEndDate\" > FM.\"FirstDrawdownDate\"  THEN 1 ";
		sql += "   	        ELSE 0  END  = 1 ";
