...
public class HlThreeLaqhcpServiceImpl extends ASpringJpaParm implements HlThreeLaqhcpService, InitializingBean {
	...
	public Slice<HlThreeLaqhcp> findAll(int index, int limit, TitaVo... titaVo) {
		...
		if (limit == Integer.MAX_VALUE)
			pageable = PageRequest.of(0, Integer.MAX_VALUE, Sort.by(Sort.Direction.ASC, "CalDate", "DeptCode", "DistCode", "UnitCode"));
		else
			pageable = PageRequest.of(index, limit, Sort.by(Sort.Direction.ASC, "CalDate", "DeptCode", "DistCode", "UnitCode"));
	...
	@Override
	public Slice<HlThreeLaqhcp> findCalDate(int calDate_0, int index, int limit, TitaVo... titaVo) {
		String dbName = "";
		Slice<HlThreeLaqhcp> slice = null;
		if (titaVo.length != 0)
			dbName = titaVo[0].getDataBase() != null ? titaVo[0].getDataBase() : ContentName.onLine;
		...
		if (dbName.equals(ContentName.onDay))
			slice = hlThreeLaqhcpReposDay.findAllByCalDateIs(calDate_0, pageable);
		else if (dbName.equals(ContentName.onMon))
			slice = hlThreeLaqhcpReposMon.findAllByCalDateIs(calDate_0, pageable);
		else if (dbName.equals(ContentName.onHist))
			slice = hlThreeLaqhcpReposHist.findAllByCalDateIs(calDate_0, pageable);
		else
			slice = hlThreeLaqhcpRepos.findAllByCalDateIs(calDate_0, pageable);
