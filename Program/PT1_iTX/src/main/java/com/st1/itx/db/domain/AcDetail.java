...
public class AcDetail implements Serializable {
	...
	// 入總帳記號
	/*
	 * 0:未入帳1:已入帳2:被沖正(隔日訂正)3.沖正(隔日訂正)9.提存傳票已上傳核心(產生傳票批號&amp;gt;=90上傳核心傳票檔時抓取EntAc=1
	 * ,並更新為9，正常傳票與沖正傳票分批上傳核心時使用)
	 */
	@Column(name = "`EntAc`")
	private int entAc = 0;
	...
	// 銷帳科目記號
	/*
	 * 0:非銷帳科目(會計科子細目設定檔帶入)1:會計銷帳科目(會計科子細目設定檔帶入)2:業務銷帳科目(會計科子細目設定檔帶入)3:未收費用、4-短繳期金、5
	 * -另收欠款(業務帶入)8:核心銷帳碼科目，須以銷帳編號(銷帳碼彙總)上傳核心(會計科子細目設定檔帶入)
	 */
	...
	// 帳冊別記號
	/*
	 * CdAcCode會計科子細目設定檔帶入(0,1,2)0:不細分，區隔帳冊固定為00A:傳統帳冊1:細分，區隔帳冊By戶號設定(AcBookCom)2:中介
	 * ，應收調撥款科目 ※明細檔無(只寫入總帳檔)，應收調撥款之核心傳票媒體檔，係自動產生。3:指定帳冊，區隔帳冊由業務交易指定
	...
	// 彙總別
	/*
	 * 撥還共用(0XX)／還款來源(1xx)／撥款方式(2xx)090:暫收抵繳091:借新還舊092:暫收轉帳093:抽退票094:轉債協暫收款095:
	 * 轉債協退還款101:匯款轉帳102:銀行扣款103:員工扣款104:支票兌現105:法院扣薪106:理賠金107:代收款-債權協商109:其他111:
	 * 匯款轉帳預先作業201:整批匯款202:單筆匯款204:退款他行(匯款單)205:核心退款(整批匯款)211:退款新光(存入憑條)
	 */
	...
	// 傳票批號
	/*
	 * 01~10:關帳11:支票繳款90~99:提存帳務，不會更新AcMain總帳檔99:利息提存98:迴轉上月96:放款承諾97:迴轉上月95:
	 * 未付火險費提存94:迴轉上月
	 */
	...
	/**
	 * 銷帳科目記號<br>
	 * 0:非銷帳科目(會計科子細目設定檔帶入) 1:會計銷帳科目(會計科子細目設定檔帶入) 2:業務銷帳科目(會計科子細目設定檔帶入)
	 * 3:未收費用、4-短繳期金、5-另收欠款(業務帶入) 8:核心銷帳碼科目，須以銷帳編號(銷帳碼彙總)上傳核心(會計科子細目設定檔帶入)
	 * 
	 * @return Integer
	 */
	...
	/**
	 * 帳冊別記號<br>
	 * CdAcCode會計科子細目設定檔帶入(0,1,2) 0:不細分，區隔帳冊固定為00A:傳統帳冊 1:細分，區隔帳冊By戶號設定(AcBookCom)
	 * 2:中介，應收調撥款科目 ※明細檔無(只寫入總帳檔)，應收調撥款之核心傳票媒體檔，係自動產生。 3:指定帳冊，區隔帳冊由業務交易指定
	 * ※L6201:其他傳票輸入、L618D各項提存
	 * 
	 * @return Integer
	 */
	...
	/**
	 * 區隔帳冊<br>
	 * 00A:傳統帳冊 201:利變年金帳冊 帳冊別記號=0時，不須處理 帳冊別記號=1時，由帳冊別處理公用程式設定 帳冊別記號=3時，由業務交易帶入
	 * 
	 * @return String
	 */
	...
	/**
	 * 彙總別<br>
	 * 撥還共用(0XX)／還款來源(1xx)／撥款方式(2xx) 090:暫收抵繳 091:借新還舊 092:暫收轉帳 093:抽退票 094:轉債協暫收款
	 * 095:轉債協退還款 101:匯款轉帳 102:銀行扣款 103:員工扣款 104:支票兌現 105:法院扣薪 106:理賠金 107:代收款-債權協商
	 * 109:其他 111:匯款轉帳預先作業 201:整批匯款 202:單筆匯款 204:退款他行(匯款單) 205:核心退款(整批匯款)
	 * 211:退款新光(存入憑條)
	 * 
	 * @return String
	 */
	...
	/**
	 * 傳票批號<br>
	 * 01~10:關帳 11:支票繳款 90~99:提存帳務，不會更新AcMain總帳檔 99:利息提存 98:迴轉上月 96:放款承諾 97:迴轉上月
	 * 95:未付火險費提存 94:迴轉上月
	 * 
	 * @return Integer
	 */
	...
	public String toString() {
		return "AcDetail [acDetailId=" + acDetailId + ", acDate=" + acDate + ", branchNo=" + branchNo + ", currencyCode=" + currencyCode + ", acNoCode=" + acNoCode + ", acSubCode=" + acSubCode
				+ ", acDtlCode=" + acDtlCode + ", acctCode=" + acctCode + ", dbCr=" + dbCr + ", txAmt=" + txAmt + ", entAc=" + entAc + ", custNo=" + custNo + ", facmNo=" + facmNo + ", bormNo="
				+ bormNo + ", rvNo=" + rvNo + ", acctFlag=" + acctFlag + ", receivableFlag=" + receivableFlag + ", acBookFlag=" + acBookFlag + ", acBookCode=" + acBookCode + ", acSubBookCode="
				+ acSubBookCode + ", sumNo=" + sumNo + ", dscptCode=" + dscptCode + ", slipNote=" + slipNote + ", slipBatNo=" + slipBatNo + ", slipNo=" + slipNo + ", titaKinbr=" + titaKinbr
				+ ", titaTlrNo=" + titaTlrNo + ", titaTxtNo=" + titaTxtNo + ", titaTxCd=" + titaTxCd + ", titaSecNo=" + titaSecNo + ", titaBatchNo=" + titaBatchNo + ", titaBatchSeq=" + titaBatchSeq
				+ ", titaSupNo=" + titaSupNo + ", titaRelCd=" + titaRelCd + ", jsonFields=" + jsonFields + ", createDate=" + createDate + ", createEmpNo=" + createEmpNo + ", lastUpdate=" + lastUpdate
