...
em = null;
  ...
  public TxTemp findSeqNoFirst(String seqNo_0, TitaVo... titaVo) {
    ...
    if (titaVo.length != 0)
      dbName = titaVo[0].getDataBase() != null ? titaVo[0].getDataBase() : ContentName.onLine;
    this.info("findSeqNoFirst " + dbName + " : " + "seqNo_0 : " + seqNo_0);
    Optional<TxTemp> txTempT = null;
    if (dbName.equals(ContentName.onDay))
      txTempT = txTempReposDay.findTopBySeqNoIsOrderByCreateDateDesc(seqNo_0);
    else if (dbName.equals(ContentName.onMon))
      txTempT = txTempReposMon.findTopBySeqNoIsOrderByCreateDateDesc(seqNo_0);
    else if (dbName.equals(ContentName.onHist))
      txTempT = txTempReposHist.findTopBySeqNoIsOrderByCreateDateDesc(seqNo_0);
    else 
      txTempT = txTempRepos.findTopBySeqNoIsOrderByCreateDateDesc(seqNo_0);
