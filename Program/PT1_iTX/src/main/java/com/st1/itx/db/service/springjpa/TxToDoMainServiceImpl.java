...
@Repository
public class TxToDoMainServiceImpl extends ASpringJpaParm implements TxToDoMainService, InitializingBean {
	@Autowired
	...
	public TxToDoMain findById(String itemCode, TitaVo... titaVo) {
		...
		this.info("findById " + dbName + " " + itemCode);
	...
	public Slice<TxToDoMain> findAll(int index, int limit, TitaVo... titaVo) {
		...
		if (limit == Integer.MAX_VALUE)
			pageable = PageRequest.of(0, Integer.MAX_VALUE, Sort.by(Sort.Direction.ASC, "ItemCode"));
		else
			pageable = PageRequest.of(index, limit, Sort.by(Sort.Direction.ASC, "ItemCode"));
	...
	public TxToDoMain excuteTxcdFirst(String autoFg_0, String excuteTxcd_1, TitaVo... titaVo) {
		...
		if (titaVo.length != 0)
			dbName = titaVo[0].getDataBase() != null ? titaVo[0].getDataBase() : ContentName.onLine;
		this.info("excuteTxcdFirst " + dbName + " : " + "autoFg_0 : " + autoFg_0 + " excuteTxcd_1 : " + excuteTxcd_1);
	...
	public TxToDoMain holdById(String itemCode, TitaVo... titaVo) {
		...
		if (titaVo.length != 0)
			dbName = titaVo[0].getDataBase() != null ? titaVo[0].getDataBase() : ContentName.onLine;
		this.info("Hold " + dbName + " " + itemCode);
		Optional<TxToDoMain> txToDoMain = null;
		if (dbName.equals(ContentName.onDay))
	...
	public TxToDoMain holdById(TxToDoMain txToDoMain, TitaVo... titaVo) {
		...
		if (titaVo.length != 0)
			dbName = titaVo[0].getDataBase() != null ? titaVo[0].getDataBase() : ContentName.onLine;
		this.info("Hold " + dbName + " " + txToDoMain.getItemCode());
		Optional<TxToDoMain> txToDoMainT = null;
		if (dbName.equals(ContentName.onDay))
	...
	public TxToDoMain insert(TxToDoMain txToDoMain, TitaVo... titaVo) throws DBException {
		...
		this.info("Insert..." + dbName + " " + txToDoMain.getItemCode());
		if (this.findById(txToDoMain.getItemCode(), titaVo) != null)
			throw new DBException(2);

		if (!empNot.isEmpty())
			txToDoMain.setCreateEmpNo(empNot);

		if (txToDoMain.getLastUpdateEmpNo() == null || txToDoMain.getLastUpdateEmpNo().isEmpty())
	...
	public TxToDoMain update(TxToDoMain txToDoMain, TitaVo... titaVo) throws DBException {
		...
		this.info("Update..." + dbName + " " + txToDoMain.getItemCode());
		if (!empNot.isEmpty())
			txToDoMain.setLastUpdateEmpNo(empNot);
	...
	public void delete(TxToDoMain txToDoMain, TitaVo... titaVo) throws DBException {
		...
		if (titaVo.length != 0)
			dbName = titaVo[0].getDataBase() != null ? titaVo[0].getDataBase() : ContentName.onLine;
		this.info("Delete..." + dbName + " " + txToDoMain.getItemCode());
		if (dbName.equals(ContentName.onDay)) {
